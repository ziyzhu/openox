...
export const CATALOG_ROOT = resolve(import.meta.dir, "../../../repositories/builtin");
...
export function listCatalogIDs(kind: CatalogKind): string[] {
  ...
  return readdirSync(root, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && ID_RE.test(entry.name)
      && existsSync(join(root, entry.name, "service.json")))
    .map((entry) => kind === "ios" ? `ios:${entry.name}` : entry.name)
    .sort();
}
