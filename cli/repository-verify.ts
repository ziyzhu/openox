...
import { readSkills } from "../service-sdk/skills.ts";
import {
  HOST_PATTERN,
  validateJSONSchemaProfile,
  validateStandardActions,
  type JSONSchema,
} from "../service-sdk/manifest.ts";
...
async function checkWebService(servicesDirectory: string, domain: string, result: Verification): Promise<void> {
  ...
  const manifest = await readJSON(manifestPath, `${label}: manifest.json`, result);
...
async function checkCatalogService(
  ...
  const label = `${kind}/${id}`;
  const manifest = await readJSON(join(servicesDirectory, id, "manifest.json"), `${label}: manifest.json`, result);
  if (manifest === undefined) return;
  if (kind === "ios") {
