...
const OPENFDA_DISCLAIMER =
  "Do not rely on openFDA to make decisions regarding medical care. Results may be incomplete or unvalidated.";
...
type LabelSectionName = keyof typeof LABEL_SECTIONS;
...
const TERMINATED_VALUES: Record<string, string> = {
  any: "All",
  yes: "1",
  no: "0",
};
...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  const boolean = (value: unknown) => value === true || String(value).toLowerCase() === "true";

  const phrase = (value: string) => `"${value.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}"`;

  const openFda = async (path: string, params: URLSearchParams) => {
    ...
    if (!Array.isArray(payload?.results)) throw new Error("openFDA returned an unexpected response");
    return { url, payload };
  };

  const pageResult = (payload: any, offset: number, returned: number) => {
  ...
  const labelSection = (record: any, name: LabelSectionName) => {
    const definition = LABEL_SECTIONS[name];
    const text = [...new Set(definition.fields.flatMap((field) => strings(record?.[field])))]
      .join("\n\n");
    const limit = 12000;
    return {
      name,
  ...
  const drugProduct = (record: any) => ({
    ...
    activeIngredients: (Array.isArray(record?.active_ingredients) ? record.active_ingredients : []).map((ingredient: any) => ({
      name: cleanText(ingredient?.name),
      strength: cleanText(ingredient?.strength),
    })),
    packages: (Array.isArray(record?.packaging) ? record.packaging : []).slice(0, 30).map((packaging: any) => ({
  ...
  const medicalDevice = (record: any) => ({
    ...
    identifiers: (Array.isArray(record?.identifiers) ? record.identifiers : []).map((identifier: any) => ({
      id: cleanText(identifier?.id),
      type: cleanText(identifier?.type),
      issuingAgency: cleanText(identifier?.issuing_agency),
    })),
    productCodes: (Array.isArray(record?.product_codes) ? record.product_codes : []).map((productCode: any) => ({
  ...
  const fetchDoc = async (url: string) => {
    const response = await retryFetch(url);
    if (!response.ok) throw new Error(`FDA returned HTTP ${response.status}`);
    return new DOMParser().parseFromString(await response.text(), "text/html");
  };

  const absoluteUrl = (value: string) => new URL(value, ORIGIN).href;

  const fdaUrl = (value: string) => {
    const url = new URL(value, ORIGIN);
    if (url.origin !== ORIGIN) throw new Error("The page must be on www.fda.gov");
    return url;
  };

  const fragmentDoc = (value: unknown) =>
    new DOMParser().parseFromString(`<body>${String(value ?? "")}</body>`, "text/html");

  const fragmentText = (value: unknown) => cleanText(fragmentDoc(value).body.textContent);

  const parseRecallRow = (row: unknown[]) => {
    const linkDoc = fragmentDoc(row[1]);
    const link = linkDoc.querySelector<HTMLAnchorElement>("a[href]");
    if (!link) return null;
  ...
  const summaryEntry = (doc: Document, label: string) => {
    const normalized = label.toLowerCase();
    const term = [...doc.querySelectorAll("dl.lcds-description-list--grid dt")].find(
      (element) => cleanText(element.textContent).replace(/:$/, "").toLowerCase() === normalized,
    );
  ...
  action("searchSite", {
    async invoke({ query, cursor, limit = 25, sort = "relevance" }) {
      ...
      const resultRoot = [...doc.querySelectorAll("main .view-content")].find((element) =>
        element.querySelector(":scope > div > div > a[href]"),
      );
      const items = [...(resultRoot?.children ?? [])]
        .filter((element) => element.tagName === "DIV")
        .map((element) => {
          const sections = [...element.children].filter((child) => child.tagName === "DIV");
          const link = sections[1]?.querySelector<HTMLAnchorElement>("a[href]");
          return {
      ...
      const nextLink = doc.querySelector<HTMLAnchorElement>(".pager__item--next a[rel='next']");
  ...
  action("listRecalls", {
    async invoke({ query = "", productType = "any", terminated = "any", cursor, limit = 25 } = {}) {
      ...
      if (!response.ok) throw new Error(`FDA recalls returned HTTP ${response.status}`);
      const payload = await response.json() as { data?: unknown[][]; recordsFiltered?: number };
      if (!Array.isArray(payload.data)) throw new Error("FDA recalls returned an unexpected response");
      const items = payload.data
        .map(parseRecallRow)
        .filter((item): item is NonNullable<ReturnType<typeof parseRecallRow>> => item !== null);
  ...
  action("getRecall", {
    async invoke({ id }) {
      ...
      if (!title) throw new Error(`No FDA recall found for ${id}`);
      ...
      const brands = [...(brandEntry?.querySelectorAll(".field--item") ?? [])]
        .map((element) => cleanText(element.textContent))
        .filter(Boolean);
      if (brands.length === 0 && brandEntry) brands.push(cleanText(brandEntry.textContent));
      ...
      for (let element = announcementHeading?.nextElementSibling; element; element = element.nextElementSibling) {
        if (element.id === "recall-photos") break;
        const sectionTitle = cleanText(element.querySelector("h2")?.textContent);
        if (sectionTitle === "Company Contact Information") break;
      ...
      return {
        ...
        companyAnnouncementDate:
          announcementDate?.querySelector("time")?.getAttribute("datetime") ?? cleanText(announcementDate?.textContent),
        fdaPublishDate:
          publishDate?.querySelector("time")?.getAttribute("datetime") ?? cleanText(publishDate?.textContent),
        ...
        productDescription: cleanText(
          productEntry?.querySelector(".field--item")?.textContent ?? productEntry?.textContent,
        ),
  ...
  action("getDrugLabel", {
    async invoke({ query, field = "any", sections = ["uses", "dosage", "warnings", "contraindications", "drugInteractions"] }) {
      ...
      if (!record) throw new Error(`No FDA drug label found for ${query}`);
      ...
      return {
        ...
        sections: (sections as LabelSectionName[]).map((name) => labelSection(record, name)),
        sourceUrl: url,
        dataLastUpdated: cleanText(payload.meta?.last_updated),
        disclaimer: cleanText(payload.meta?.disclaimer) || OPENFDA_DISCLAIMER,
        labelingNotice:
          "This is submitted labeling data and may not match currently distributed or FDA-approved labeling.",
      };
    },
  });
  ...
  action("searchDrugShortages", {
    async invoke({ query, status = "Current", cursor, limit = 20 }) {
      ...
      const items = payload.results.map((record: any) => ({
  ...
  action("lookupDrug", {
    async invoke({ ndc, brandName, genericName, cursor, limit = 20 }) {
      const offset = pageCursor(cursor, 0);
      const search = ndc
        ? `(product_ndc:${phrase(ndc)} OR packaging.package_ndc:${phrase(ndc)})`
        : brandName
          ? `brand_name:${phrase(brandName)}`
          : `generic_name:${phrase(genericName)}`;
  ...
  action("lookupDevice", {
    async invoke({ udi, brandName, catalogNumber, cursor, limit = 20 }) {
      const offset = pageCursor(cursor, 0);
      const search = udi
        ? `identifiers.id:${phrase(udi)}`
        : brandName
          ? `brand_name:${phrase(brandName)}`
          : `catalog_number:${phrase(catalogNumber)}`;
      const params = new URLSearchParams({ search, skip: String(offset), limit: String(limit) });
  ...
  action("getPage", {
    async invoke({ path }) {
      ...
      const main = (doc.querySelector("main#main") || doc.querySelector("main") || doc.body).cloneNode(true) as Element;
      main
        .querySelectorAll("script, style, noscript, nav, aside, form, iframe, .lcds-breadcrumb, .region-pre-header")
        .forEach((element) => element.remove());
      main
        .querySelectorAll("h1, h2, h3, h4, p, li, dt, dd, tr")
        .forEach((element) => element.after(doc.createTextNode("\n")));
      ...
      const links = [...main.querySelectorAll<HTMLAnchorElement>("a[href]")]
        ...
        .slice(0, 80);
      if (!title || !text) throw new Error("FDA page did not contain readable content");
