...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  const fetchJson = async (url: string, init: RequestInit = {}) => {
    ...
    if (!response.ok) throw new Error(`Perplexity returned HTTP ${response.status}`);
    try {
  ...
  const accountHeaders = async () => {
    ...
    if (!id) throw new Error("Not signed in to Perplexity");
    return { "x-pplx-account": String(id) };
  };

  const requestHeaders = (url: string, reason: string, extra: Record<string, string> = {}) => ({
  ...
  const threadEndpoint = (id: string, cursor: string | undefined, limit: number) => {
  ...
  const placesFrom = (blocks: any[]) => {
    const places = blocks
      ?.flatMap((block: any) => block?.maps_mode_block?.places || [])
      .filter((place: any) => place && typeof place.name === "string") || [];
    return places.map((place: any) => ({
      name: place.name,
      url: typeof place.url === "string" ? place.url : "",
      address: Array.isArray(place.address) ? place.address.filter((value: unknown) => typeof value === "string").join(", ") : "",
  ...
  const answerFrom = (record: any, threadId?: string) => {
    const blocks = Array.isArray(record?.blocks) ? record.blocks : [];
    const answerBlock = blocks.find((block: any) => typeof block?.markdown_block?.answer === "string");
    ...
    return {
      ...
      relatedQueries: Array.isArray(record?.related_queries)
        ? record.related_queries.filter((value: unknown) => typeof value === "string")
        : [],
  ...
  const parseEventStream = (text: string) => {
    const messages = text
      .split(/\r?\n/)
      ...
      .filter(Boolean);
    const final = messages.findLast((message: any) => message?.final_sse_message === true)
      || messages.findLast((message: any) => message?.text_completed === true)
      || messages.findLast((message: any) => message?.backend_uuid);
    if (!final) throw new Error("Perplexity did not return a completed answer");
    return final;
  };
  ...
  action("askQuestion", {
    async invoke({ query }: { query: string }) {
      ...
      const entry = Array.isArray(thread?.entries)
        ? thread.entries.find((candidate: any) => candidate?.uuid === streamed.id) || thread.entries.at(-1)
        : null;
  ...
  action("listThreads", {
    async invoke() {
      ...
      if (!Array.isArray(data)) throw new Error("Perplexity returned an unexpected thread list");
      const items = data
        .filter((thread: any) => typeof thread?.uuid === "string")
        .map((thread: any) => ({
          id: thread.uuid,
  ...
  action("getThread", {
    async invoke({ id, cursor, limit = 10 }: { id: string; cursor?: string; limit?: number }) {
      ...
      const result = {
        ...
        entries: data.entries.map((entry: any) => answerFrom(entry, id)),
