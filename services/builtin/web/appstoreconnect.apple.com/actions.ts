...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  const parseJson = (text: string, context: string) => {
    try {
  ...
  const session = async () => {
    ...
    if (!id) throw new Error("App Store Connect has no active provider");
    return { body, team: { id: String(id), type: "PURPLESOFTWARE" } as Team };
  };

  const headers = (team: Team, extra: Record<string, string> = {}) => ({
    ...JSON_HEADERS,
  ...
  const requestJson = async (path: string, team: Team, init: RequestInit = {}) => {
    const response = await retryFetch(`${ORIGIN}${path}`, {
      credentials: "include",
      ...init,
      headers: headers(team, init.headers as Record<string, string> | undefined),
    });
    const text = await response.text();
  ...
  const cursorFrom = (body: any) => {
    if (body?.meta?.paging?.nextCursor) return String(body.meta.paging.nextCursor);
  ...
  const imageUrl = (asset: any) => {
    const template = asset?.templateUrl;
    if (!template) return null;
    return String(template).replace("{w}", "256").replace("{h}", "256").replace("{f}", "png");
  };

  const includedById = (body: any) => new Map<string, any>((body?.included ?? []).map((item: any) => [String(item.id), item]));

  const mapVersion = (item: any) => {
  ...
  const mapApp = (item: any, included: Map<string, any>) => {
    const attributes = item?.attributes ?? {};
    const versionIds = item?.relationships?.displayableVersions?.data ?? [];
    const versions = versionIds.map((reference: any) => included.get(reference.id)).filter(Boolean).map(mapVersion);
  ...
  const mapBuild = (item: any, included: Map<string, any>) => {
    const attributes = item?.attributes ?? {};
  ...
  action("getSignInState", {
    async invoke() {
      const { response, text } = await rawSession();
      if (response.status === 401 || response.status === 403 || response.status === 302) return { signedIn: false };
      if (response.status < 200 || response.status >= 300) throw new Error(`App Store Connect session HTTP ${response.status}`);
      try {
        const body = JSON.parse(text);
  ...
  action("listApps", {
    async invoke({ cursor, limit }: { cursor?: string; limit?: number }) {
      ...
      const items = (body?.data ?? []).map((item: any) => mapApp(item, included));
  ...
  action("getApp", {
    async invoke({ id }: { id: string }) {
      ...
      if (!body?.data) throw new Error(`App not found: ${id}`);
  ...
  action("listPreReleaseVersions", {
    async invoke({ appId, platform, cursor, limit }: { appId: string; platform?: string; cursor?: string; limit?: number }) {
      const { team } = await session();
      const query = new URLSearchParams({
        "filter[app]": appId,
        sort: "-version",
  ...
  action("listBuilds", {
    async invoke({ appId, platform, processingState, cursor, limit }: { appId: string; platform?: string; processingState?: string; cursor?: string; limit?: number }) {
      const { team } = await session();
      const query = new URLSearchParams({
        "filter[app]": appId,
      ...
      if (platform) query.set("filter[preReleaseVersion.platform]", platform);
      if (processingState) query.set("filter[processingState]", processingState);
      if (cursor) query.set("cursor", cursor);
      const body = await requestJson(`/iris/v1/builds?${query}`, team);
      const included = includedById(body);
      const items = (body?.data ?? []).map((item: any) => mapBuild(item, included));
      const nextCursor = cursorFrom(body);
  ...
  action("listBetaGroups", {
    async invoke({ appId }: { appId: string }) {
      const { team } = await session();
  ...
  action("getAppAnalytics", {
    async invoke({ appId, startDate, endDate, frequency, metrics }: { appId: string; startDate: string; endDate: string; frequency?: string; metrics?: string[] }) {
      const { team } = await session();
      ...
      const results = (body?.results ?? []).map((result: any) => ({
        ...
        points: (result?.data ?? []).map((point: any) => ({ date: String(point?.date ?? ""), value: Number(point?.value ?? 0) })),
      }));
