...
const normalizedLabel = (value: unknown) => cleanText(value)
  .replace(/[：:]$/, "")
  .toLowerCase();

const englishText = (element: Element | null) => {
  if (!element) return "";
...
const labelElement = (root: ParentNode, label: string) => [...root.querySelectorAll("p.en, span.en")]
  .find((element) => normalizedLabel(element.textContent) === normalizedLabel(label));

const fieldCell = (root: ParentNode, label: string) => {
  const labelCell = labelElement(root, label)?.closest("td");
  if (!labelCell) return null;
  if (labelCell.nextElementSibling) return labelCell.nextElementSibling;
...
const fieldText = (root: ParentNode, label: string) => englishText(fieldCell(root, label));
...
const tablesInField = (root: ParentNode, label: string) => {
  const cell = fieldCell(root, label);
  if (!cell) return [];
  return [...cell.querySelectorAll(":scope > table, :scope > div > table")];
};

const ageValue = (root: ParentNode, label: string) => {
...
const install: ActionInstaller = ({ action, log }) => {
  action("searchTrials", {
    async invoke({
      ...
      if (!query) throw new Error("searchTrials: query is required");
      ...
      const items = rows.map((row) => {
        ...
        const link = row.querySelector<HTMLAnchorElement>("a[href*='showprojEN.html?proj=']");
  ...
  action("getTrial", {
    async invoke({ id }: { id: string }) {
      if (!id) throw new Error("getTrial: id is required");
      ...
      if (!title || !registrationNumber) throw new Error(`getTrial: no trial found for id ${id}`);
