...
const install: ActionInstaller = ({ action, retryFetch }) => {
  ...
  const fetchText = async (path: string) =>
    (await retryFetch(path, { credentials: "include" })).text();

  const matchBrace = (html: string, from: number) => {
    ...
    for (let j = from; j < html.length; j++) {
      ...
      else if (c === "}" && --depth === 0) return html.slice(from, j + 1);
    }
    throw new Error("unterminated JSON object");
  };

  const islandVar = (html: string, name: string) => {
  ...
  const times = (t: any) =>
    t ? { scheduled: t.scheduled ?? null, estimated: t.estimated ?? null, actual: t.actual ?? null } : null;

  const flightSummary = (f: any) => ({
    ident: f.ident,
  ...
  const parseBoard = (doc: Document, type: string) => {
    ...
    return [...table.querySelectorAll("tr")]
      ...
      .map((tr) => {
        ...
        const text = (el?: Element | null) =>
          ((el as HTMLElement | null)?.innerText ?? "").replace(/\u00a0/g, " ").replace(/\s+/g, " ").trim();
        const ident = text(tr.querySelector(".flight-ident a"));
        const aircraftType = text(cells[1]);
        const placeCode = (cells[2]?.querySelector<HTMLAnchorElement>('a[href^="/live/airport"]')?.innerText ?? "").trim();
  ...
  action("getFlight", {
    async invoke({ ident }) {
      if (!ident) throw new Error("getFlight: ident required");
      ...
      if (!flights.length) throw new Error(`getFlight: no flight for ${ident}`);
  ...
  action("getAirport", {
    async invoke({ airport }) {
      if (!airport) throw new Error("getAirport: airport code required");
  ...
  action("getAirportStats", {
    async invoke({ airport, days = 30 }) {
      if (!airport) throw new Error("getAirportStats: airport code required");
      const res = await retryFetch(
        `/ajax/ignoreuser/airport_stats.rvt?airport=${encodeURIComponent(airport)}`,
        { credentials: "include", headers: { "x-requested-with": "XMLHttpRequest" } },
      );
      const d = await res.json();
      return {
        airport: airport.toUpperCase(),
        daily: (d.chart_data ?? []).slice(0, Math.max(1, days)).map((c: any) => ({ date: c.date, arrivals: c.arrivals, departures: c.departures })),
