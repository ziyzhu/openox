...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  const jpmcHeaders = (post = false): Record<string, string> => {
    const h: Record<string, string> = {
      "x-jpmc-channel": "id=C30",
      "x-jpmc-csrf-token": "NONE",
      "x-jpmc-client-request-id": crypto.randomUUID(),
    };
    if (post) h["content-type"] = "application/x-www-form-urlencoded; charset=UTF-8";
    return h;
  };

  const postForm = async (path: string, body?: Record<string, string>): Promise<any> => {
  ...
  const signedOut = () =>
    new Error("Chase session is signed out. Run getSignInUrl, sign in, then retry.");

  const fetchBootstrap = async (): Promise<any> =>
    postForm("/svc/rl/accounts/l4/v1/app/data/list");

  const accountTilesOf = (bootstrap: any): any[] => {
    const cache: any[] = Array.isArray(bootstrap?.cache) ? bootstrap.cache : [];
  ...
  action("listAccounts", {
    async invoke() {
      const data = await fetchBootstrap();
      if (!data?.personId) throw signedOut();
      return { items: accountTilesOf(data).map(mapTile), nextCursor: null };
    },
  });

  action("getAccount", {
    async invoke({ accountId } = {} as any) {
      if (!accountId) throw new Error("getAccount requires accountId");
      const data = await postForm("/svc/rr/accounts/secure/v2/account/detail/card/list", {
        accountId: String(accountId),
      });
      if (data?.code && data.code !== "SUCCESS") throw new Error(`Chase returned ${data.code}`);
      const d = data?.detail ?? {};
      return {
  ...
  action("listTransactions", {
    async invoke({ accountId, limit = 50, cursor, startDate, endDate } = {} as any) {
      if (!accountId) throw new Error("listTransactions requires accountId");
      ...
      if (cursor) params.set("pagination-contextual-text", String(cursor));
      const data = await getJson(
        `/svc/rr/accounts/secure/gateway/credit-card/transactions/inquiry-maintenance/etu-transactions/v4/accounts/transactions?${params}`,
      );
  ...
  action("listPayees", {
    async invoke() {
      ...
      const items = Object.values(payees).map((p: any) => ({
  ...
  action("createPayment", {
    async invoke({ payeeId, amount, paymentDate, fundingAccountId, paymentOption } = {} as any) {
      if (!payeeId) throw new Error("createPayment requires payeeId");

      const optionsData = await postForm("/svc/rr/payments/secure/v2/billpay/multi/payment/add/options", {
        payeeId: String(payeeId),
      });
      if (optionsData?.code && optionsData.code !== "SUCCESS") throw new Error(`Chase returned ${optionsData.code}`);
      const opts = optionsData?.creditCardMultiPayBillOptions ?? {};
      const fundingAccounts: any[] = Array.isArray(opts.fundingAccounts) ? opts.fundingAccounts : [];
      const fundingOptions: any[] = Array.isArray(opts.fundingOptions) ? opts.fundingOptions : [];
      const dateOptions: string[] = Array.isArray(opts.dateOptions) ? opts.dateOptions : [];
      ...
      if (!account) throw new Error("No matching funding account; call listAccounts/options to pick a fundingAccountId");
      ...
      if (!chosen) chosen = usable.find((o) => o?.optionType === "OTHER_AMOUNT");
      if (!chosen) throw new Error("No usable payment option for this payee");

      const payAmount = amount != null ? Number(amount) : chosen?.amount;
      if (payAmount == null) throw new Error("createPayment requires amount for OTHER_AMOUNT");

      const payDate = paymentDate ? String(paymentDate) : dateOptions[0];
      if (!payDate) throw new Error("No available payment date");
      ...
      if (validateData?.errorStatus) throw new Error("Chase rejected the payment at validation");
      const formId = validateData?.formId;
      if (!formId) throw new Error("Chase did not return a formId; payment not submitted");

      const result = await postForm("/svc/wr/payments/secure/v2/billpay/multi/payment/add", {
        formId: String(formId),
      });
      if (result?.code && result.code !== "SUCCESS") throw new Error(`Chase returned ${result.code}`);
