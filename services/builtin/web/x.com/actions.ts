...
export const extractArticleBody = (articleResults: any): ArticleBody => {
  ...
  for (const media of mediaEntities) {
    ...
    if (id && typeof url === "string" && url) mediaById.set(id, url);
  }

  const entityMap = new Map<string, any>();
  const entities = articleResults?.content_state?.entityMap;
  const entityEntries = Array.isArray(entities)
    ? entities.map((entry: any) => [String(entry?.key ?? ""), entry?.value])
    : Object.entries(entities || {});
  for (const [key, value] of entityEntries) entityMap.set(key, value);
...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  type OpMeta = { queryId: string; features: Record<string, boolean>; fieldToggles: Record<string, boolean> };
  ...
  const FALLBACK_TWEET_FEATURES: Record<string, boolean> = {
  ...
  const FALLBACK_USER_FEATURES: Record<string, boolean> = {
  ...
  const opCache: Record<string, OpMeta> = {};

  const quotedKeys = (raw: string | undefined): string[] =>
    raw ? Array.from(raw.matchAll(/"([^"]+)"/g)).map((m) => m[1]) : [];
  const keysToFlags = (keys: string[]): Record<string, boolean> =>
  ...
  const OP_MODULE_RE =
    /queryId:"([A-Za-z0-9_-]+)",operationName:"([A-Za-z0-9_]+)",operationType:"\w+",metadata:\{featureSwitches:\[([^\]]*)\],fieldToggles:\[([^\]]*)\]\}/g;

  const harvestOps = (text: string): number => {
    let m: RegExpExecArray | null;
    let count = 0;
    OP_MODULE_RE.lastIndex = 0;
    while ((m = OP_MODULE_RE.exec(text))) {
      const [, queryId, name, feats, toggles] = m;
      if (opCache[name]) continue;
      opCache[name] = {
        queryId,
  ...
  let manifestUrls: string[] | null = null;
  const fetchManifestUrls = async (): Promise<string[]> => {
    if (manifestUrls) return manifestUrls;
    try {
      const sw = await (await fetch("/sw.js")).text();
      manifestUrls = Array.from(
        new Set(Array.from(sw.matchAll(/https?:\/\/\S*?client-web\/[A-Za-z0-9_~.-]+\.js/g)).map((m) => m[0])),
      );
    } catch {
      manifestUrls = [];
    }
  ...
  const rankFor = (url: string, opHint: string): number => {
    const file = (url.split("/").pop() || "").toLowerCase();
    if (/^main\.[0-9a-f]+\.js$/.test(file)) return 0;
    if (opHint && file.includes(opHint)) return 1;
    if (/^(loader|shared~).*(card|timeline)/.test(file)) return 2;
    if (/^(loader|shared~|bundle)\./.test(file)) return 3;
    return 4;
  };

  const candidateScripts = (operationName: string, manifest: string[]): string[] => {
  ...
  const scrapeOperationFromBundle = async (operationName: string): Promise<OpMeta | null> => {
    ...
    for (let i = 0; i < urls.length && !opCache[operationName]; i += BATCH) {
      ...
      for (const text of texts) if (text.includes('operationName:"')) harvestOps(text);
    }
    const hit = opCache[operationName];
    if (hit) log(`x.com: scraped ${operationName}=${hit.queryId} from live bundle`);
    return hit ?? null;
  };

  const resolveOperation = async (operationName: string, baselineFeatures: Record<string, boolean>): Promise<OpMeta> => {
    if (opCache[operationName]) return opCache[operationName];
    const found = await scrapeOperationFromBundle(operationName);
    if (!found) throw new Error(`x.com: could not resolve a live queryId for ${operationName} from the bundle`);
    const features = Object.keys(found.features).length > 0 ? found.features : baselineFeatures;
    const op: OpMeta = { queryId: found.queryId, features, fieldToggles: found.fieldToggles };
    opCache[operationName] = op;
    return op;
  };

  const resolveTweetOp = (op: string) => resolveOperation(op, FALLBACK_TWEET_FEATURES);
  const resolveUserOp = (op: string) => resolveOperation(op, FALLBACK_USER_FEATURES);

  const getCt0 = (): string | null => {
    const m = document.cookie.match(/(?:^|;\s*)ct0=([^;]+)/);
    return m ? decodeURIComponent(m[1]) : null;
  };

  const authHeaders = (ct0: string, post = false): Record<string, string> => {
  ...
  const requireCt0 = (): string => {
    const ct0 = getCt0();
    if (!ct0) throw new Error("Not signed in to x.com (no ct0 cookie). Sign in first.");
    return ct0;
  };

  const normalizeHandle = (raw: string | undefined | null): string => {
  ...
  const parseTweetId = (input: string): string => {
  ...
  const fetchOwnHandle = async (): Promise<string | null> => {
    const ct0 = getCt0();
    if (!ct0) return null;
    try {
  ...
  const extractMedia = (legacy: any): { has_media: boolean; media_urls: string[] } => {
    const media = legacy?.extended_entities?.media || legacy?.entities?.media;
    if (!Array.isArray(media) || media.length === 0) return { has_media: false, media_urls: [] };
    const urls: string[] = [];
    for (const m of media) {
      if (!m) continue;
      if (m.type === "video" || m.type === "animated_gif") {
        const variants = m.video_info?.variants || [];
        const mp4 = variants.find((v: any) => v?.content_type === "video/mp4");
  ...
  const extractCard = (tweet: any): any | null => {
    ...
    const matched = cardUrlBinding
      ? urlEntities.find((e: any) => e?.url === cardUrlBinding || e?.expanded_url === cardUrlBinding)
      : undefined;
  ...
  const extractQuotedTweet = (tweet: any): any | null => {
    ...
    const qScreen =
      qUser?.legacy?.screen_name?.trim?.() ||
      qUser?.core?.screen_name?.trim?.() ||
      "";
    if (!qScreen || !/^[A-Za-z0-9_]{1,15}$/.test(qScreen)) return null;
  ...
  const tweetRow = (result: any, seen: Set<string>): any | null => {
  ...
  const walkInstructions = (
    ...
    const recurse = (value: any) => {
      ...
      visit(value);
      if (Array.isArray(value)) { for (const v of value) recurse(v); return; }
      for (const child of Object.values(value)) if (child && typeof child === "object") recurse(child);
    };
  ...
  const graphqlGet = (op: OpMeta, operation: string, variables: any, ct0: string) =>
    fetchJson(buildGraphqlUrl(op.queryId, operation, variables, op.features, op.fieldToggles), {
      headers: authHeaders(ct0),
      credentials: "include",
    });

  const graphqlPost = (op: OpMeta, operation: string, variables: any, ct0: string) =>
    fetchJson(`/i/api/graphql/${op.queryId}/${operation}`, {
      method: "POST",
  ...
  const lookupUserId = async (screen: string, ct0: string): Promise<string> => {
    ...
    if (d?.__http_error) throw new Error(`UserByScreenName HTTP ${d.__http_error} for @${screen} (queryId may be stale)`);
    const id = d?.data?.user?.result?.rest_id;
    if (!id) throw new Error(`X user @${screen} not found`);
    return String(id);
  };

  const resolveTargetHandle = async (input: string | undefined): Promise<string> => {
    const h = normalizeHandle(input);
    if (h) return h;
    const own = await fetchOwnHandle();
    if (!own) throw new Error("Could not detect logged-in x.com user (no AppTabBar handle in /home)");
    return own;
  };
  ...
  action("getProfile", {
    async invoke({ username }: { username?: string } = {}) {
      ...
      const op = await resolveUserOp("UserByScreenName");
      const d = await graphqlGet(op, "UserByScreenName", { screen_name: screen, withSafetyModeUserFields: true }, ct0);
      if (d?.__http_error) throw new Error(`getProfile HTTP ${d.__http_error} for @${screen} (queryId may be stale)`);
  ...
  const collectTweets = async (
    ...
    limit: number,
    fetchPage: (cursor: string | null, count: number) => Promise<any>,
    selectInstructions: (data: any) => any[],
    errLabel: string,
    ...
    for (let page = 0; page < 20 && rows.length < limit; page++) {
      ...
      if (data?.__http_error) {
        if (rows.length === 0) throw new Error(`${errLabel}: HTTP ${data.__http_error} (queryId may be stale)`);
        break;
      }
      ...
      if (rows.length === beforeLen) break;
  ...
  action("listTweets", {
    async invoke({ username, cursor: startCursor, limit = 20 }: { username?: string; cursor?: string; limit?: number } = {}) {
      const ct0 = requireCt0();
      const screen = await resolveTargetHandle(username);
      ...
      return await collectTweets(
        ...
        limit,
        (cursor, count) => {
          const vars: any = { userId, count, includePromotedContent: false, withQuickPromoteEligibilityTweetFields: true, withVoice: true };
  ...
  action("searchTweets", {
    async invoke({ query, product = "top", from, has, exclude, cursor: startCursor, limit = 15 }: any) {
      const ct0 = requireCt0();
      const parts = [String(query || "").trim()];
      if (from) parts.push(`from:${normalizeHandle(from)}`);
      if (has) parts.push(`filter:${has}`);
      if (exclude) {
        ...
        if (map[exclude]) parts.push(map[exclude]);
      }
      const rawQuery = parts.filter(Boolean).join(" ");
      if (!rawQuery) throw new Error("searchTweets: empty query");
      const productMap: Record<string, string> = { top: "Top", live: "Latest", photos: "Photos", videos: "Videos" };
      ...
      return await collectTweets(
        ...
        (d) => d?.data?.search_by_raw_query?.search_timeline?.timeline?.instructions || [],
        `searchTweets ${JSON.stringify(rawQuery)}`,
      );
    },
  });

  action("listTimeline", {
    async invoke({ type = "for-you", cursor: startCursor, limit = 20 }: { type?: "for-you" | "following"; cursor?: string; limit?: number } = {}) {
      const ct0 = requireCt0();
  ...
  action("listThread", {
    async invoke({ tweetId, cursor: startCursor, limit = 50 }: { tweetId: string; cursor?: string; limit?: number }) {
      const ct0 = requireCt0();
      ...
      for (let pageNum = 0; pageNum < 5 && rows.length < limit; pageNum++) {
        ...
        if (data?.__http_error) {
          if (rows.length === 0) throw new Error(`listThread HTTP ${data.__http_error} for tweet ${id} (not found or queryId stale)`);
          break;
        }
        const instructions =
  ...
  action("listTrending", {
    async invoke() {
      try {
        ...
        const instructions =
          data?.data?.explore_page?.body?.initialTimeline?.timeline?.timeline?.instructions ?? [];
        const trends: any[] = [];
        for (const ins of instructions) {
          for (const ent of ins?.entries ?? []) {
            const entryId: string = ent?.entryId ?? "";
            // Top-of-page story cells are not trends; skip them and promos.
            if (!entryId.startsWith("trend-")) continue;
  ...
  action("listBookmarks", {
    async invoke({ cursor: startCursor, limit = 20 }: { cursor?: string; limit?: number } = {}) {
      const ct0 = requireCt0();
      ...
      return await collectTweets(
        ...
        limit,
        (cursor, count) => {
          const vars: any = { count, includePromotedContent: false };
          if (cursor) vars.cursor = cursor;
  ...
  action("listLikes", {
    async invoke({ username, cursor: startCursor, limit = 20 }: { username?: string; cursor?: string; limit?: number } = {}) {
      ...
      return await collectTweets(
        ...
        limit,
        (cursor, count) => {
          const vars: any = { userId, count, includePromotedContent: false, withClientEventToken: false, withBirdwatchNotes: false, withVoice: true };
          if (cursor) vars.cursor = cursor;
  ...
  const userRow = (result: any): any | null => {
  ...
  const collectUsers = async (
    ...
    for (let page = 0; page < 20 && rows.length < limit; page++) {
      ...
      const data = await fetchPage(cursor, count);
      if (data?.__http_error) {
        if (rows.length === 0) throw new Error(`${errLabel}: HTTP ${data.__http_error} (queryId may be stale or list private)`);
        break;
      }
      const instructions =
      ...
      const { nextCursor } = walkInstructions(instructions, (n) => {
        if (n.user_results?.result) {
          ...
          if (u && !seen.has(u.screen_name)) { seen.add(u.screen_name); rows.push(u); }
        }
      });
      lastCursor = nextCursor;
      if (rows.length === before) break;
  ...
  action("listVerifiedFollowers", {
    async invoke({ username, cursor: startCursor, limit = 50 }: { username?: string; cursor?: string; limit?: number } = {}) {
      const ct0 = requireCt0();
      const screen = await resolveTargetHandle(username);
      const userId = await lookupUserId(screen, ct0);
      const op = await resolveTweetOp("BlueVerifiedFollowers");
      return await collectUsers(
        startCursor ?? null,
        limit,
        (cursor, count) => {
          const vars: any = { userId, count, includePromotedContent: false };
          if (cursor) vars.cursor = cursor;
  ...
  action("listFollowing", {
    async invoke({ username, cursor: startCursor, limit = 50 }: { username?: string; cursor?: string; limit?: number } = {}) {
      ...
      return await collectUsers(
        ...
        limit,
        (cursor, count) => {
          const vars: any = { userId, count, includePromotedContent: false, withClientEventToken: false, withBirdwatchNotes: false, withVoice: true, withV2Timeline: true };
          if (cursor) vars.cursor = cursor;
  ...
  action("listNotifications", {
    async invoke({ cursor: startCursor, limit = 20 }: { cursor?: string; limit?: number } = {}) {
      ...
      const vars: any = { count: Math.min(100, Math.max(20, limit + 10)), includePromotedContent: false };
      if (startCursor) vars.cursor = startCursor;
      const data = await graphqlGet(op, "NotificationsTimeline", vars, ct0);
      if (data?.__http_error) throw new Error(`listNotifications HTTP ${data.__http_error} (queryId may be stale)`);
      const instructions =
        data?.data?.viewer?.notification_all_list?.timeline?.instructions ||
      ...
      const out: any[] = [];
      const seen = new Set<string>();
      const processItem = (itemContent: any, entryId: string) => {
        ...
        if (item.__typename === "TimelineNotification") {
          ...
          if (target) {
            ...
            text += text && tt ? " | " + tt : tt;
            if (!urlStr) urlStr = `https://x.com/i/status/${target.rest_id}`;
          }
  ...
  action("getArticle", {
    async invoke({ tweetId }: { tweetId: string }) {
      const ct0 = requireCt0();
      const id = parseTweetId(tweetId);
      ...
      if (data?.__http_error) throw new Error(`getArticle HTTP ${data.__http_error} for tweet ${id} (not found or queryId stale)`);
      const result = data?.data?.tweetResult?.result;
      if (!result) throw new Error(`getArticle: tweet ${id} not found`);
      const tw = result.tweet || result;
      const legacy = tw.legacy || {};
      const u = tw.core?.user_results?.result;
      const screen = u?.legacy?.screen_name || u?.core?.screen_name || "unknown";
