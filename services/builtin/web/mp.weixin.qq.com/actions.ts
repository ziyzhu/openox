...
const positiveInteger = (...values: Array<string | null>) => {
...
const articleText = (root: Element, indexes: Map<Element, number>) => {
...
const install: ActionInstaller = ({ action, retryFetch }) => {
  action("getArticle", {
    async invoke({ url: inputUrl }) {
      ...
      if (!response.ok) throw new Error(`getArticle: HTTP ${response.status}`);
      ...
      const coverImageUrl = imageUrl(document.querySelector<HTMLMetaElement>('meta[property="og:image"]'));
      return {
        title,
        ...
        summary: cleanText(document.querySelector<HTMLMetaElement>('meta[property="og:description"]')?.content),
