...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  const graphql = async (operationName: string, query: string, variables: unknown = {}) => {
    const response = await retryFetch(
      `${ORIGIN}/graphql/${operationName}?operation=${encodeURIComponent(operationName)}`,
      {
        method: "POST",
        credentials: "include",
        headers: {
    ...
    if (!response.ok) throw new Error(`DoorDash ${operationName} HTTP ${response.status}`);
    let body: any;
    try {
  ...
  const fetchStore = async (storeId: string) => {
    ...
    if (!details) throw new Error(`DoorDash store ${storeId} was not found`);
  ...
  const fetchCart = async () => {
    ...
    const items = (cart.orders ?? []).flatMap((order: any) =>
  ...
  const fetchOrders = async (limit: number) => {
    ...
    if (!response.ok) throw new Error(`DoorDash order history HTTP ${response.status}`);
    const doc = new DOMParser().parseFromString(html, "text/html");
    const cards = [...doc.querySelectorAll('[data-testid="OrderHistoryOrderItem"]')];
    const seen = new Set<string>();
    const items: { id: string; url: string; summary: string }[] = [];
    for (const card of cards) {
      const link = [...card.querySelectorAll("a[href]")].find((candidate) =>
        /^\/orders\/[0-9a-f-]+$/i.test(new URL(candidate.getAttribute("href") ?? "", ORIGIN).pathname),
      );
      if (!link) continue;
  ...
  const fetchReceipt = async (orderCartId: string) => {
    ...
    if (!receipt) throw new Error(`DoorDash order ${orderCartId} was not found`);
    const items = (receipt.orders ?? []).flatMap((order: any) =>
      (order?.orderItemsList ?? []).map((orderItem: any) => {
        ...
        return {
          ...
          options: (orderItem.optionsList ?? [])
            .map((option: any) => clean(option?.itemExtraOption?.name))
            .filter(Boolean),
    ...
    const lineItems = (receipt.lineItems ?? []).map((lineItem: any) => ({
  ...
  action("getSignInState", {
    async invoke() {
      const response = await retryFetch(
        `${ORIGIN}/unified-gateway/notification_preferences/v1/doordash/consumer`,
        {
          credentials: "include",
      ...
      if (!response.ok) throw new Error(`DoorDash sign-in probe HTTP ${response.status}`);
      return { signedIn: true };
    },
  });

  action("getStore", {
    async invoke({ storeId } = {} as any) {
      if (!clean(storeId)) throw new Error("getStore requires storeId");
      return { store: await fetchStore(clean(storeId)) };
    },
  });

  action("searchStoreItems", {
    async invoke({ storeId, query, cursor, limit = 20 } = {} as any) {
      if (!clean(storeId)) throw new Error("searchStoreItems requires storeId");
      if (!clean(query)) throw new Error("searchStoreItems requires query");
      ...
      for (const facet of result?.legoRetailItems ?? []) {
        if (facet?.component?.category !== "card.retail_item" || !clean(facet.custom)) continue;
        try {
  ...
  action("getOrder", {
    async invoke({ orderCartId } = {} as any) {
      if (!clean(orderCartId)) throw new Error("getOrder requires orderCartId");
  ...
  action("getPaymentState", {
    async invoke({ previousOrderId } = {} as any) {
      ...
      try {
        ...
        log(
          `getPaymentState order verification unavailable: ${
