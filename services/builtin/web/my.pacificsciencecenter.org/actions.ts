...
const localTimestamp = (date: Date) =>
  `${dateString(date)} ${pad(date.getHours())}:${pad(date.getMinutes())}:${pad(date.getSeconds())}`;

const parseDate = (value: string, endOfDay: boolean) => {
  const date = new Date(`${value}T${endOfDay ? "23:59:59" : "00:00:00"}`);
  if (Number.isNaN(date.getTime()) || dateString(date) !== value) throw new Error(`Invalid date: ${value}`);
  return date;
};

const requiredToken = (doc: Document) => {
  const token = doc.querySelector<HTMLInputElement>('input[name="__RequestVerificationToken"]')?.value;
  if (!token) throw new Error("Pacific Science Center did not provide a request token");
  return token;
};
...
const ticketType = (ticket: HTMLElement) => {
  ...
  if (!priceText) throw new Error(`Missing price for ${label || "ticket type"}`);
  const select = ticket.querySelector<HTMLSelectElement>(".tn-ticket-selector__pricetype-select");
  return {
...
const eventConfiguration = (doc: Document) => {
  for (const script of doc.scripts) {
    const source = script.textContent ?? "";
    if (!source.includes("new window.tnew.EventDetail")) continue;
    const match = source.match(/new window\.tnew\.EventDetail\(\s*({[\s\S]*?})\s*\)/);
    if (!match) continue;
    const configuration = JSON.parse(match[1]!);
    if (configuration?.performanceConfiguration) return configuration.performanceConfiguration;
  }
  throw new Error("Showtime page did not provide its reservation configuration");
};

const cartDetails = (doc: Document) => {
  const items = [...doc.querySelectorAll<HTMLElement>(".tn-cart-item")].map((item) => ({
    ...
    tickets: [...item.querySelectorAll<HTMLElement>(".tn-cart-item-detail__list")].map((detail) => {
  ...
  const millisecondsRemaining = Number(doc.querySelector<HTMLElement>(".tn-count-down-timer")?.dataset.tnMillisecondsRemaining);
...
const receiptDetails = (doc: Document, id: string) => {
  ...
  if (!orderNumber || orderNumber !== id || !cart.items.length) throw new Error(`Receipt ${id} was not found`);
  const orderDate = cleanText(doc.querySelector(".tn-order-date")?.textContent).replace(/^Order Date:\s*/, "");
  return { id, orderDate, ...cart };
};

const parsedOrderDate = (value: string) => {
  const date = new Date(value.replace(/(\d)(AM|PM)$/, "$1 $2"));
  return Number.isNaN(date.getTime()) ? null : date;
};

const isSince = (orderDate: string, since?: string) => {
  if (!since) return true;
  const order = parsedOrderDate(orderDate);
  const start = new Date(since.replace(" ", "T"));
  if (!order || Number.isNaN(start.getTime())) throw new Error("Invalid transaction date scope");
...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  action("searchEvents", {
    async invoke({ query, startDate, endDate }) {
      ...
      if (!needle) throw new Error("query must contain visible text");
      ...
      if (!Array.isArray(productions)) throw new Error("Event search returned an unexpected response");

      const items = productions.flatMap((production: any) => {
  ...
  action("getShowtime", {
    async invoke({ productionSeasonId, performanceId }) {
      ...
      const zones = [...doc.querySelectorAll<HTMLInputElement>(".tn-ticket-selector__input-zone")].map((input) => {
        const id = input.dataset.zoneId ?? input.value;
        const container = doc.querySelector<HTMLElement>(`.tn-ticket-selector__pricetype-container[data-zone-id="${id}"]`);
        const ticketTypes = container
          ? [...container.querySelectorAll<HTMLElement>(".tn-ticket-selector__pricetype")].map(ticketType)
          : [];
  ...
  action("updateCart", {
    async invoke({ productionSeasonId, performanceId, zoneId, tickets }) {
      if (!tickets.length) throw new Error("At least one ticket type is required");
      if (new Set(tickets.map((ticket: any) => ticket.ticketTypeId)).size !== tickets.length) {
        throw new Error("Each ticket type may appear only once");
      }
      const pageUrl = `${BASE}/${encodeURIComponent(productionSeasonId)}/${encodeURIComponent(performanceId)}`;
      const doc = await fetchDocument(pageUrl);
      const zone = doc.querySelector<HTMLInputElement>(`.tn-ticket-selector__input-zone[data-zone-id="${CSS.escape(zoneId)}"]`);
      const container = doc.querySelector<HTMLElement>(`.tn-ticket-selector__pricetype-container[data-zone-id="${CSS.escape(zoneId)}"]`);
      if (!zone || !container) throw new Error(`Section ${zoneId} is not available for this showtime`);
      const availableTickets = new Map(
        [...container.querySelectorAll<HTMLElement>(".tn-ticket-selector__pricetype")]
          .map(ticketType)
      ...
      for (const requested of tickets) {
        ...
        if (!available) throw new Error(`Ticket type ${requested.ticketTypeId} is not available in section ${zoneId}`);
      ...
      if (maximumQuantity && quantity > maximumQuantity) throw new Error(`This showtime allows at most ${maximumQuantity} tickets per order`);
      ...
      if (!response.ok) throw new Error(`Ticket reservation: HTTP ${response.status}`);
      ...
      if (!cart.items.length) throw new Error("Ticket reservation succeeded, but the cart could not be read");
  ...
  action("getPaymentState", {
    async invoke({ since }) {
      ...
      if (liveReceiptId) {
        ...
        if (!isSince(receipt.orderDate, since)) return { status: "none", reference: null };
