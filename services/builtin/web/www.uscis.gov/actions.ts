...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  action("getPage", {
    async invoke({ path } = {}) {
      ...
      if (!url.startsWith(ORIGIN)) throw new Error("path must be on www.uscis.gov");
  ...
  const renderSearch = async (query: string) => {
    ...
    const ready = () =>
      doc && doc.querySelector(".gsc-webResult.gsc-result, .gs-no-results-result, .gsc-results");
  ...
  action("searchSite", {
    async invoke({ query } = {}) {
      ...
      try {
        ...
        iframe = r.iframe;
        const doc = r.doc;
        if (!doc) throw new Error("search did not render");
  ...
  action("listNews", {
    async invoke({ cursor } = {}) {
      ...
      const items = [...doc.querySelectorAll(".views-row")]
        .map((row) => {
          const a = row.querySelector<HTMLAnchorElement>(".views-field-title a");
          return {
  ...
  action("getForm", {
    async invoke({ form } = {}) {
      ...
      const scope =
        doc.querySelector(".field--name-field-attached-files") ||
        doc.querySelector("main#main") ||
        doc.body;
      const seen = new Set<string>();
      const files = [...scope.querySelectorAll<HTMLAnchorElement>("a[href$='.pdf']")]
        .map((a) => {
          const info =
            a.closest(".media--type-document, .file")?.querySelector(".file-ext-info, .extra-info") ||
            null;
          return {
      ...
      if (files.length === 0) throw new Error(`no downloadable PDF found for ${form} at ${url}`);
