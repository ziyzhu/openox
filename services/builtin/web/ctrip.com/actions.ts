...
const install: ActionInstaller = ({ action, log }) => {
  ...
  const flightSpaPath = (from: string, to: string, date: string) =>
    `/list/oneway-${from}-${to}?depdate=${date}&cabin=Y_S_C_F&adult=1&child=0&infant=0`;

  const flightRouter = () => {
    if (!/flights\.ctrip\.com/.test(location.href)) return null;
    return (window as any).next?.router ?? null;
  };

  const dismissFlightModal = () => {
    const btn = Array.from(document.querySelectorAll("button,div,span,a")).find((e) =>
      (e as HTMLElement).offsetParent && (e.textContent || "").trim() === "我知道了");
    (btn as HTMLElement | undefined)?.click();
  };

  const flightPageState = (): "captcha" | "content" | "pending" => {
    dismissFlightModal();
    if (/captcha/.test(location.pathname)
      || /验证码|verify the human|拖动滑块/i.test(document.body?.innerText || "")) return "captcha";
    ...
    document.querySelectorAll(".flight-list > span > div").forEach((card) => {
      ...
      const walk = (node: Node) => {
        for (const c of Array.from(node.childNodes)) {
          if (c.nodeType === 3) { const t = clean(c.textContent || ""); if (t) chunks.push(t); }
      ...
      for (let i = 0; i < chunks.length - 1; i++) {
        if (isCurrency(chunks[i]) && isPriceDigits(chunks[i + 1])) {
          price = Number(chunks[i + 1].replace(",", "")); break;
        }
      }
      let cabin = "";
    ...
    async invoke({ query }: { query: string }) {
      try {
        ...
        for (const g of j?.recommendGroupList ?? []) {
          ...
          if (Array.isArray(cl)) cities.push(...cl);
        }
        const q = String(query ?? "").trim().toLowerCase();
        const seen = new Set<string>();
        const items = cities
          .filter((c) =>
            !q || [c.name, c.nameEn, c.namePy, c.code].some((s) =>
  ...
  action("getFlightPriceCalendar", {
    async invoke({ fromCity, toCity, startDate, returnDate }: {
      fromCity: string; toCity: string; startDate: string; returnDate?: string;
    }) {
      try {
        ...
        const prices = (j?.priceList ?? []).map((p: any) => ({
  ...
  action("listFlightDeals", {
    async invoke({ fromCity, fromName }: { fromCity: string; fromName?: string }) {
      try {
        const now = new Date();
        ...
        const items = (j?.routes ?? []).map((rt: any) => {
          const cheapest = (rt.pl ?? []).reduce(
            (a: any, b: any) => (!a || (b.price ?? Infinity) < a.price ? b : a), null);
          return {
  ...
  action("searchHotels", {
    async invoke({ cityId, checkIn, checkOut, page }: {
      cityId: number; checkIn: string; checkOut: string; page?: number;
    }) {
      try {
        ...
        const list: any[] = j?.data?.hotelList ?? [];
  ...
  action("searchFlights", {
    async invoke({ from, to, date }: { from: string; to: string; date: string }) {
      try {
        ...
        if (!onTarget) {
          ...
          setTimeout(() => { try { router.push(flightSpaPath(f, t, date)); } catch {} }, 50);
          w.__cflTries = 0;
          return { items: [], pending: true, note: "" };
        }
        let state = flightPageState();
        for (let i = 0; i < 10 && state !== "content"; i++) { await sleep(1200); state = flightPageState(); }
        if (state === "captcha")
