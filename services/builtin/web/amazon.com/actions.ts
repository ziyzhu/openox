...
export const visibleText = (root: Node | null | undefined): string => {
  if (!root) return "";
  const parts: string[] = [];
  const visit = (node: Node) => {
    if (node.nodeType === 1 && NON_CONTENT_TEXT_TAGS.has((node as Element).tagName.toUpperCase())) return;
    if (node.nodeType === 3) {
...
export const amazonOrderId = (input: string | null | undefined): string | null => {
  const match = clean(input).match(/(?:D01-\d{7}-\d{7}|\d{3}-\d{7}-\d{7})/i)?.[0];
  return match?.toUpperCase() ?? null;
};

export const amazonOrderDateText = (headerText: string | null | undefined): string | null => {
  const match = clean(headerText).match(/^(?:Order placed|Ordered on|Subscription charged on)\s+(.+)$/i);
  return match?.[1] || null;
};

export const amazonOrderSearchPath = (query: string, cursor?: string): string => {
  const search = clean(query);
  if (!search) throw new Error("searchOrders requires a query");
  const page = pageCursor(cursor, 1);
  const params = new URLSearchParams({ search });
  if (page === 1) params.set("opt", "ab");
  else params.set("page", String(page));
  return `/your-orders/search?${params}`;
};

export const amazonOrderHasData = (order: {
  orderId?: unknown;
  placedDate?: unknown;
  placedText?: unknown;
  totalText?: unknown;
  ...
  order.status ||
  order.statusDetail ||
  order.orderDetailsUrl ||
  order.items?.length
);

export const amazonOrderPage = <T extends Parameters<typeof amazonOrderHasData>[0]>(orders: T[]) => ({
...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  const RANKING_ROOTS: Record<string, string> = {
  ...
  const extractAsin = (input: string | null | undefined): string | null => {
    const s = clean(input);
    if (!s) return null;
  ...
  const productUrlOf = (asin: string) => `${ORIGIN}/dp/${asin}`;

  const parsePrice = (text: string | null | undefined) => {
    const s = clean(text);
    const m = s.match(/([$€£])\s*(\d+(?:,\d{3})*(?:\.\d+)?)/);
    if (!m) return { priceText: s || null, priceValue: null, currency: null };
  ...
  const parseRating = (text: string | null | undefined): number | null => {
    const m = clean(text).match(/(\d+(?:\.\d+)?)\s*out of 5/i);
    return m ? Number.parseFloat(m[1]) : null;
  };

  const parseReviewCount = (text: string | null | undefined): number | null => {
    const s = clean(text);
    const compact = s.match(/(\d+(?:\.\d+)?)\s*([kKmM])/);
    if (compact) {
      const value = Number.parseFloat(compact[1]);
      const mult = /m/i.test(compact[2]) ? 1_000_000 : 1_000;
  ...
  const isAmazonEntity = (text: string | null | undefined) =>
    clean(text).toLowerCase().includes("amazon");
  ...
  const extractShipsFrom = (text: string | null | undefined): string | null => {
  ...
  const extractSoldBy = (text: string | null | undefined): string | null => {
  ...
  const fetchDoc = async (path: string, diagnostic?: string): Promise<Document> => {
    ...
    const haystack = clean(
      (doc.title || "") + " " + (doc.querySelector("body")?.textContent || ""),
    ).toLowerCase();
  ...
  const imageOf = (el: Element | null | undefined): string =>
    abs(el?.getAttribute("src") || el?.getAttribute("data-src") || el?.getAttribute("data-old-hires"));

  const linesSeparatedByBreaks = (root: Node | null | undefined): string[] => {
    if (!root) return [];
    const lines = [""];
    const visit = (node: Node) => {
      if (node.nodeType === 1) {
        const tag = (node as Element).tagName.toUpperCase();
        if (NON_CONTENT_TEXT_TAGS.has(tag)) return;
        if (tag === "BR") {
          lines.push("");
          return;
        }
  ...
  const isoDate = (text: string | null | undefined): string | null => {
  ...
  const csrfFrom = (doc: ParentNode): string =>
    clean(
      doc.querySelector("meta[name='anti-csrftoken-a2z']")?.getAttribute("content") ||
        doc.querySelector("input[name='anti-csrftoken-a2z']")?.getAttribute("value"),
    );

  const decodeOnce = (v: string): string =>
    /%[0-9A-Fa-f]{2}/.test(v) ? (() => { try { return decodeURIComponent(v); } catch { return v; } })() : v;

  const cartOffer = (card: ParentNode) => {
    const form = card.querySelector("form[action*='/cart/add-to-cart']") as Element | null;
    const action = form?.getAttribute("action") || "";
    const brandId = action.match(/local-market\/([^/?]+)/)?.[1] || null;
    const cartType = brandId ? "LOCAL_MARKET" : form ? "RETAIL" : null;
    const field = (name: string) => clean(form?.querySelector(`input[name='${name}']`)?.getAttribute("value"));
    if (!form) return {
      offerListingId: null as string | null,
      brandId: null as string | null,
      addToCartPath: null as string | null,
      cartType: null as string | null,
      merchantId: null as string | null,
      minOrderQuantity: null as number | null,
      maxOrderQuantity: null as number | null,
    };
    const raw = clean(
      form.querySelector("input[name='items[0.base][offerListingId]'], input[name*='offerListingId' i]")?.getAttribute("value") ||
        card.querySelector("[data-offer-listing-id]")?.getAttribute("data-offer-listing-id"),
    );
  ...
  const formBody = (pairs: [string, string][]) =>
    pairs.map(([k, v]) => k + "=" + enc(v)).join("&");

  const postForm = (url: string, csrf: string, pairs: [string, string][]) =>
    retryFetch(url, {
      method: "POST",
      credentials: "include",
      headers: {
  ...
  const fetchAuthedDoc = async (path: string, diagnostic?: string): Promise<Document> => {
  ...
  const probeSignedIn = async (): Promise<boolean> => {
  ...
  action("searchProducts", {
    async invoke({ query, cursor, limit = 20, department, sort = "featured" } = {} as any) {
      if (!query) throw new Error("searchProducts requires a query");
      const page = pageCursor(cursor, 1);
      ...
      if (items.length === 0) log("searchProducts: no product cards for " + query + " (page " + page + ")");
  ...
  action("listDepartments", {
    async invoke() {
      ...
      for (const opt of options) {
        ...
        if (!id || id === "aps" || byId.has(id)) continue;
        byId.set(id, label);
      }
      const departments = [...byId].map(([id, label]) => ({ id, label }));
      if (departments.length === 0) log("listDepartments: no department options found");
      return { departments };
    },
  });

  action("getProduct", {
    async invoke({ input } = {} as any) {
      const asin = extractAsin(input);
      if (!asin) throw new Error("getProduct requires an ASIN or product URL");
      const doc = await fetchDoc(`/dp/${asin}`);
      const title = clean(
        doc.querySelector("#productTitle, #title span")?.textContent,
      );
      if (!title) {
      ...
      const ratingText =
        ...
        doc.querySelector("#acrPopover")?.textContent ||
        "";
      ...
      return {
        ...
        breadcrumbs: uniqueNonEmpty(
          [...doc.querySelectorAll("#wayfinding-breadcrumbs_feature_div a")].map((a) => a.textContent),
        ),
        bullets: uniqueNonEmpty(
          [...doc.querySelectorAll("#feature-bullets li .a-list-item")].map((li) => li.textContent),
        ),
      };
    },
  });
  ...
  action("getOffer", {
    async invoke({ input } = {} as any) {
      const asin = extractAsin(input);
      if (!asin) throw new Error("getOffer requires an ASIN or product URL");
      const doc = await fetchDoc(`/dp/${asin}`);
      const merchantInfo = clean(doc.querySelector("#merchant-info")?.textContent);
      const shipsBlob = clean(
        doc.querySelector("#shipsFromSoldByInsideBuyBox_feature_div")?.textContent ||
          doc.querySelector("#fulfillerInfoFeature_feature_div")?.textContent ||
          doc.querySelector("#merchantInfoFeature_feature_div")?.textContent ||
          doc.querySelector("#tabular-buybox-container")?.textContent ||
          "",
      );
      const soldBy =
        clean(doc.querySelector("#sellerProfileTriggerId")?.textContent) ||
        extractSoldBy(shipsBlob) ||
  ...
  action("listReviews", {
    async invoke({ input, cursor, limit = 10 } = {} as any) {
      const asin = extractAsin(input);
      if (!asin) throw new Error("listReviews requires an ASIN or product URL");
      const page = pageCursor(cursor, 1);
      ...
      const parse = (doc: Document) => {
        ...
        const qaUrls = [
          ...new Set(
            [...doc.querySelectorAll('a[href*="ask/questions"]')]
              .map((a) => (a as HTMLAnchorElement).getAttribute("href") || "")
              .filter(Boolean)
              .map((href) => (href.startsWith("http") ? href : ORIGIN + href)),
          ),
        ];
        const samples = [...doc.querySelectorAll('[data-hook="review"]')]
          .slice(0, limit)
          .map((card) => {
            const ratingText =
              ...
              "";
            return {
              title: clean(card.querySelector('[data-hook="review-title"]')?.textContent)
                .replace(/^\d+(?:\.\d+)?\s*out of 5 stars\s*/i, "") || null,
              ratingValue: parseRating(ratingText),
        ...
        const hasNext = !!doc.querySelector(
          'li.a-last:not(.a-disabled) a, [data-hook="pagination-bar"] li.a-last:not(.a-disabled) a',
        );
        return {
      ...
      const nextCursor =
        usedReviewsPage && parsed.hasNext && parsed.samples.length >= limit
          ? String(page + 1)
          : null;
      return {
        asin,
  ...
  action("listOrders", {
    async invoke({ timeFilter = "months-3", orderFilter = "orders", cursor, limit = 10 } = {} as any) {
      const start = pageCursor(cursor, 0);

      const headerValue = (card: Element, label: string): string => {
        for (const item of card.querySelectorAll(".order-header__header-list-item")) {
          const t = clean(item.textContent);
          if (t.toLowerCase().startsWith(label.toLowerCase())) return clean(t.slice(label.length));
        }
        return "";
      };

      const parseCard = (card: Element) => {
        const detailsHref = abs(card.querySelector('a[href*="order-details"]')?.getAttribute("href"));
        const orderId = (detailsHref && new URL(detailsHref).searchParams.get("orderID")) ||
          headerValue(card, "Order #") || null;
        const placedText = [...card.querySelectorAll(".order-header__header-list-item")]
          .map((item) => amazonOrderDateText(item.textContent))
          .find(Boolean) || null;
      ...
      while (consumed < limit && pageHasMore) {
        ...
        if (orderFilter === "orders") params.set("timeFilter", timeFilter);
        else params.set("orderFilter", orderFilter);
        ...
        log(
          `${diagnostic}: cards=${cards.length} selected=${parsed.length} valid=${valid.length} empty=${parsed.length - valid.length} ` +
      ...
      if (items.length === 0) log(`listOrders: no order cards (orderFilter ${orderFilter}, timeFilter ${timeFilter})`);
  ...
  action("searchOrders", {
    async invoke({ query, cursor } = {} as any) {
      const path = amazonOrderSearchPath(query, cursor);
      const doc = await fetchAuthedDoc(path, `searchOrders: page=${pageCursor(cursor, 1)}`);
      const records = [...doc.querySelectorAll(".a-section.a-spacing-large.a-spacing-top-large")]
        .filter((record) =>
          record.querySelector('a[href*="order-details"]') &&
          record.querySelector('a[href*="/dp/"], a[href*="/gp/product/"]')
        );
      const items = records.map((record) => {
        ...
        const placedText = [...record.querySelectorAll("*")]
          .filter((node) => node.children.length === 0)
          .map((node) => amazonOrderDateText(node.textContent))
          .find(Boolean) || null;
        return {
  ...
  action("getOrderDetails", {
    async invoke({ input } = {} as any) {
      ...
      if (!orderId) throw new Error("getOrderDetails requires an order id like 114-1234567-1234567 or an order-details URL");
      const allowedPaths = new Set([
        "/your-orders/order-details",
        "/gp/css/order-details",
        "/uff/your-account/order-details",
      ]);
      let requestPath = orderId.startsWith("D01-")
        ? `/gp/css/order-details?orderID=${encodeURIComponent(orderId)}`
        : `/your-orders/order-details?orderID=${encodeURIComponent(orderId)}`;
      ...
      if (freshRows.length > 0) {
        const orderedText = [...doc.querySelectorAll("#order-summary .a-color-tertiary")]
          .map((el) => clean(el.textContent))
          .find((text) => /^Ordered\s+/i.test(text)) || "";
        const orderDateText = orderedText
          .replace(/^Ordered\s+/i, "")
          .replace(/\s+\d{1,2}:\d{2}\s*(?:AM|PM)$/i, "") || null;
        const address = [...doc.querySelectorAll("#delivery-destination .a-size-base")]
        ...
        const charges = [...doc.querySelectorAll("#order-summary .ufpo-charge-breakdown > .a-row")]
          .map((row) => {
            const label = clean(row.querySelector("dt")?.textContent).replace(/:\s*$/, "");
            const amountCandidates = [...row.querySelectorAll("dd span")]
              .filter((span) => !span.classList.contains("a-text-strike"))
        ...
        return {
          ...
          paymentMethod: payBrand
            ? payBrand + (payTail ? ` ending in ${payTail}` : "")
            : clean(payList?.textContent) || null,
          charges,
          items,
          shipments: [],
      ...
      const comp = (name: string) => doc.querySelector(`[data-component=${name}]`);

      const orderDateText = clean(comp("orderDate")?.textContent) || null;

      const shipToLines = [...(comp("shippingAddress")?.querySelectorAll("li") ?? [])]
        .map((li) => clean(li.textContent))
        .filter(Boolean);

      const charges = [...doc.querySelectorAll("#od-subtotals .a-row")]
        .map((row) => clean(row.textContent).match(/^(.+?):\s*(-?\$[\d,.]+)$/))
        .filter(Boolean)
        .map((m) => ({
          label: m![1],
          amountText: m![2],
      ...
      const items = [...doc.querySelectorAll("[data-component=itemTitle]")].map((titleEl) => {
        ...
        const asin = extractAsin(link?.getAttribute("href") || "");
        const within = (name: string) => clean(row.querySelector(`[data-component=${name}]`)?.textContent);
  ...
  action("trackPackage", {
    async invoke({ input } = {} as any) {
      ...
      for (const row of container?.querySelectorAll(".a-row") ?? []) {
        ...
        if (m) events.push({ date, time: m[1], description: m[2] || null });
      }
      const status = clean(
        doc.querySelector("#primaryStatus")?.textContent ||
          [...doc.querySelectorAll("h1, h2, h3")].map((h) => clean(h.textContent))
            .find((t) => /delivered|arriving|out for delivery|shipped|in transit/i.test(t)),
      ) || null;
  ...
  const cartLineDetails = (doc: Document) => {
    ...
    for (const line of doc.querySelectorAll("#sc-active-cart [data-itemid][data-asin]")) {
      ...
      const detail = {
        ...
        priceText: Number.isFinite(priceValue)
          ? (currency === "USD" ? `$${priceValue.toFixed(2)}` : String(priceValue))
          : null,
        priceValue: Number.isFinite(priceValue) ? priceValue : null,
        currency,
        selected: line.hasAttribute("data-isselected")
          ? line.getAttribute("data-isselected") === "1"
          : null,
        inStock: line.hasAttribute("data-outofstock")
          ? line.getAttribute("data-outofstock") !== "1"
          : null,
  ...
  action("getCart", {
    async invoke() {
      const res = await retryFetch(
        ORIGIN + "/cart/add-to-cart/get-cart-items?clientName=SiteWideActionExecutor",
        { credentials: "include" },
      );
      ...
      const subtotals = uniqueNonEmpty(
        (clean(doc.querySelector("#sc-cart-column, #sc-page-content")?.textContent)
          .match(/Subtotal[^:]*:\s*\$[\d,.]+/g) ?? []),
      );
  ...
  action("deleteCartItem", {
    async invoke({ itemId } = {} as any) {
      itemId = clean(itemId);
      if (!/^[\w-]+$/.test(itemId)) throw new Error("deleteCartItem requires a cart line id from getCart");
      const doc = await fetchAuthedDoc("/gp/cart/view.html");
      ...
      if (!target || !asin) throw new Error("deleteCartItem: the cart line no longer exists");
      const numberAttr = (line: Element, name: string) =>
        Number.parseFloat(clean(line.getAttribute(name))) || 0;
      ...
      if (!ok) throw new Error("deleteCartItem: Amazon did not confirm that the item was removed");
  ...
  action("addToCart", {
    async invoke(args = {} as any) {
      ...
      const asin = extractAsin(input);
      if (!asin) throw new Error("addToCart requires an ASIN or product URL");
      ...
      const card = [...searchDoc.querySelectorAll('[data-component-type="s-search-result"]')]
        .find((candidate) => extractAsin(candidate.getAttribute("data-asin")) === asin);
      ...
      const path = clean(addToCartPath) || found.addToCartPath ||
        (type === "LOCAL_MARKET" ? `/cart/add-to-cart/local-market/${brand}/ref=ox_atc` : "/cart/add-to-cart?ref=ox_atc");
      const rawOffer = clean(args.offerListingId) || found.offerListingId || "";
      const oli = type === "LOCAL_MARKET" ? decodeOnce(rawOffer) : rawOffer;
      if (!oli) throw new Error("addToCart: no purchasable offer found for this product");
      ...
      if (type === "RETAIL") {
        ...
        if (merchant) pairs.push(["merchantId", merchant]);
      }
  ...
  const checkoutSummary = (doc: Document) => {
    const text = clean(doc.querySelector("body")?.textContent);
    const grab = (label: RegExp) => text.match(label)?.[1] || null;
    return {
  ...
  action("getCheckout", {
    async invoke({ brandId } = {} as any) {
      const brand = clean(brandId) || FRESH_BRAND;
      const { doc, purchaseId, csrf } = await loadCheckout(brand);
      if (!purchaseId) throw new Error("getCheckout: no checkout in progress — add items to the cart first");
      return { purchaseId, csrf, brandId: brand, ...checkoutSummary(doc) };
    },
  });

  action("setTip", {
    async invoke({ amount, purchaseId, csrf } = {} as any) {
      const value = Number(amount);
      if (!Number.isFinite(value) || value < 0) throw new Error("setTip requires a non-negative amount");
      purchaseId = clean(purchaseId);
      csrf = clean(csrf);
      if (!purchaseId || !csrf) throw new Error("setTip requires purchaseId and csrf from getCheckout");
  ...
  action("placeOrder", {
    async invoke({ purchaseId, csrf } = {} as any) {
      purchaseId = clean(purchaseId);
      csrf = clean(csrf);
      if (!purchaseId || !csrf) throw new Error("placeOrder requires purchaseId and csrf from getCheckout");
      const res = await postForm(
        `${ORIGIN}/checkout/p/${purchaseId}/spc/place-order?pipelineType=Chewbacca&purchasePrograms=FRESH&referrer=spc`,
        csrf,
        [
          ["anti-csrftoken-a2z", csrf],
    ...
    async invoke({ limit = 30 } = {} as any) {
      ...
      for (const el of doc.querySelectorAll("[data-asin]")) {
        ...
        if (entry.priceText == null) {
          ...
          if (price.priceValue != null) Object.assign(entry, price);
        }
        byAsin.set(asin, entry);
      }
      const items = [...byAsin.values()].filter((i) => i.title).slice(0, limit);
      if (items.length === 0) log("listBuyAgain: no buy-again items");
  ...
  action("listWishlists", {
    async invoke() {
      const res = await retryFetch(
        ORIGIN + "/nav/ajax/wishlist?wishlistItems=wishlist&ajaxTemplate=wishlist",
        { credentials: "include" },
      );
      const raw = await res.text();
      let lists: any[] = [];
      try {
        ...
        for (const group of data?.wishlistItems?.template?.data?.items ?? []) {
          for (const it of group?.items ?? []) {
            ...
            if (id) lists.push({ id, name: clean(it.text), url });
          }
        }
      } catch {
  ...
  action("listWishlistItems", {
    async invoke({ list, cursor, limit = 30 } = {} as any) {
      const id = clean(list).match(/(?:\/wishlist\/ls\/)?([A-Z0-9]{11,15})/i)?.[1];
      if (!id) throw new Error("listWishlistItems requires a wishlist id or URL from listWishlists");
      const params = new URLSearchParams({ viewType: "list" });
      if (cursor) params.set("lek", cursor);
      ...
      for (const li of doc.querySelectorAll("#g-items li")) {
        ...
        items.push({
          ...
          addedText: clean(li.querySelector("[id^=itemAddedDate], .dateAddedText")?.textContent)
            .replace(/^Item added\s*/i, "") || null,
        });
        if (items.length >= limit) break;
      }
      const lek = doc.querySelector("input[name=lastEvaluatedKey]")?.getAttribute("value");
      const done = !!doc.querySelector("#endOfListMarker");
      if (items.length === 0 && !done) log("listWishlistItems: no items parsed for " + id);
  ...
  action("listSubscriptions", {
    async invoke() {
      const res = await retryFetch(
        ORIGIN + "/auto-deliveries/ajax/subscriptionList?deviceType=desktop&deviceContext=web",
        { credentials: "include" },
      );
      const doc = new DOMParser().parseFromString(await res.text(), "text/html");
      if (clean(doc.title).toLowerCase().includes("sign-in")) {
      ...
      const items = [...doc.querySelectorAll(".subscription-card")]
        .filter((card) => !card.querySelector(".store-front-ingress-message-container") &&
          !(card.getAttribute("class") || "").includes("store-front-ingress"))
        .map((card) => {
          const link = card.querySelector('a[href*="/dp/"], a[href*="/auto-deliveries/"]');
          return {
            title: clean(card.querySelector("img")?.getAttribute("alt")) ||
              clean(link?.getAttribute("title")) || null,
  ...
  action("getPrimeMembership", {
    async invoke() {
      ...
      const pairValue = (label: string): string | null => {
        for (const el of doc.querySelectorAll("[class*=mc-menu-head-container]")) {
          const t = clean(el.textContent);
          if (t.toLowerCase().startsWith(label.toLowerCase())) return clean(t.slice(label.length)) || null;
        }
        return null;
      };
      const planBox = [...doc.querySelectorAll("[class*=box-lumix]")]
        .find((box) => /current membership/i.test(box.textContent || ""));
  ...
  action("listRankingCategories", {
    async invoke({ list = "bestsellers" } = {} as any) {
      const root = ORIGIN + (RANKING_ROOTS[list] || RANKING_ROOTS.bestsellers);
      const doc = await fetchDoc(root);
      const seen = new Set<string>();
      const items = [...doc.querySelectorAll(
        "#zg-left-col a[href*='/zgbs/'], #zg-left-col a[href*='/new-releases/'], #zg-left-col a[href*='/movers-and-shakers/'], .zg-browse-group a",
      )].flatMap((link) => {
      ...
      if (items.length === 0) log("listRankingCategories: no categories for " + list);
      return { items, nextCursor: null };
    },
  });

  action("listRankings", {
    async invoke({ list = "bestsellers", category, cursor, limit = 30 } = {} as any) {
      const root = clean(category)
        ? abs(category)
        : ORIGIN + (RANKING_ROOTS[list] || RANKING_ROOTS.bestsellers);
      if (!root) throw new Error("listRankings requires a category URL returned by listRankingCategories");
      const seen = new Set<string>();
      const items: any[] = [];
      ...
      for (let pg = firstPage; pg < firstPage + 4 && items.length < limit; pg++) {
        ...
        for (const card of cards) {
          ...
          const asin =
            extractAsin(card.getAttribute("data-asin") || "") ||
            extractAsin(link?.getAttribute("href") || "");
          ...
          if (!title) continue;
          const rankText = clean(
            card.querySelector(".zg-bdg-text, [class*='rank']")?.textContent,
          );
          const price = parsePrice(
            card.querySelector(".a-price .a-offscreen, .a-color-price")?.textContent,
          );
          const ratingText = card.querySelector('[aria-label*="out of 5 stars"]')
          ...
          items.push({
            ...
            reviewCount: parseReviewCount(
              card.querySelector('a[href*="#customerReviews"], .a-size-small')?.textContent,
            ),
          });
          added++;
          if (items.length >= limit) break;
      ...
      if (items.length === 0) log("listRankings: no ranked items for " + list);
