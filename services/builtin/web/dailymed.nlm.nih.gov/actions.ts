...
const textList = (element: Element | null) => {
  if (!element) return [];
...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  action("searchDrugLabels", {
    async invoke({ query, field = "all", labelType = "all", cursor, limit = 20 }: {
      ...
      if (!query) throw new Error("searchDrugLabels: query is required");
      ...
      const items = [...doc.querySelectorAll(".results article.row")].map((row) => {
        const link = row.querySelector<HTMLAnchorElement>("a.drug-info-link");
      ...
      const nextHref = doc.querySelector<HTMLAnchorElement>(".pagination a.next-link")?.getAttribute("href");
  ...
  action("getDrugLabel", {
    async invoke({ id }: { id: string }) {
      if (!id) throw new Error("getDrugLabel: id is required");
      ...
      if (!title) throw new Error(`getDrugLabel: no label found for id ${id}`);
      ...
      const pdfUrl = absoluteUrl(doc.querySelector<HTMLAnchorElement>("a.pdf")?.getAttribute("href") ?? null);
      const xmlZipUrl = absoluteUrl(doc.querySelector<HTMLAnchorElement>("a.xml")?.getAttribute("href") ?? null);
      const officialLabelUrl = absoluteUrl(doc.querySelector<HTMLAnchorElement>("a[href*='fdaDrugXsl.cfm']")?.getAttribute("href") ?? null);
      const result = {
        id,
        title,
