...
const string = (value: any) => value == null ? "" : String(value);
const optionalString = (value: any) => value == null || value === "" ? null : String(value);
...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  const withNavigation = async <T>(work: () => Promise<T>): Promise<T> => {
    const previous = navigationTail;
    let release = () => {};
    navigationTail = new Promise<void>((resolve) => {
      release = resolve;
    });
  ...
  const requestJson = async (url: string): Promise<any> => {
    ...
    if (!response.ok) throw new Error(`ModMed HTTP ${response.status}`);
    try {
  ...
  const patientId = async () => {
    ...
    if (!Number.isInteger(id) || id <= 0) throw new Error("ModMed patient record is unavailable");
    return id;
  };

  const portalReady = () => {
    if (location.hostname !== "frontierdermpartners.modmedapp.com") return false;
    if (!location.pathname.startsWith(PORTAL)) return false;
  ...
  const captureRoute = async (path: string, pattern: RegExp, label: string) =>
    withNavigation(async () => {
      const capture = (window as CaptureWindow).oxFetchCapture;
      if (!capture) throw new Error("ModMed response capture is unavailable");
  ...
  const inheritedOrFallback = async (
    path: string,
    pattern: RegExp,
    label: string,
    fallback: () => Promise<any>,
  ) => {
    if (portalReady()) return captureRoute(path, pattern, label);
    try {
      const body = await fallback();
      log(`${label}: used direct response path`);
      return body;
    } catch (error) {
      if (await waitForPortal()) return captureRoute(path, pattern, label);
      throw error;
    }
  };
  ...
  action("getSignInState", {
    async invoke() {
      if (portalReady()) return { signedIn: true };
      try {
        const header = await requestJson(patientHeaderUrl());
        return { signedIn: Boolean(header?.id && header?.portalEnabled !== false) };
      } catch (error) {
        if (/sign-in required \(HTTP (?:401|403)\)/.test(String((error as Error)?.message ?? error))) {
          return { signedIn: await waitForPortal() };
        }
        if (await waitForPortal()) return { signedIn: true };
        throw error;
      }
    },
  });
  ...
  action("getPatientProfile", {
    async invoke() {
      const body = await inheritedOrFallback(
        `${PORTAL}/my-health/contact-info`,
        /\/ema\/ws\/v3\/portal\/patient\/\d+\/info(?:\?|$)/,
        "getPatientProfile",
        async () => {
  ...
  action("listPastAppointments", {
    async invoke() {
      const body = await inheritedOrFallback(
        `${PORTAL}/appointments/past-appointments`,
        /\/ema\/ws\/v3\/patientPortal\/appointments\/past(?:\?|$)/,
      ...
      const items = (Array.isArray(body) ? body : []).map((visit: any) => ({
  ...
  action("getMedicationHistory", {
    async invoke() {
      const body = await inheritedOrFallback(
        `${PORTAL}/my-health/medications`,
        /\/ema\/ws\/v3\/portal\/patient\/\d+\/medicalHistory(?:\?|$)/,
  ...
  action("listProblems", {
    async invoke() {
      ...
      const items = (Array.isArray(body) ? body : []).map((problem: any) => ({
  ...
  action("getAllergySummary", {
    async invoke() {
      const body = await inheritedOrFallback(
        `${PORTAL}/my-health/allergies`,
        /\/ema\/ws\/v3\/portal\/patient\/\d+\/allergies(?:\?|$)/,
        "getAllergySummary",
        async () => {
          const id = await patientId();
    ...
    async invoke() {
      ...
      if (portalReady()) {
        ...
        try {
          ...
          if (!await waitForPortal()) throw error;
          [balances, statements] = await captureBilling();
        }
      }
      const accounts = (Array.isArray(balances) ? balances : []).flatMap((configuration: any) =>
        (Array.isArray(configuration?.patientBalanceInfoBeanList) ? configuration.patientBalanceInfoBeanList : [])
          .map((balance: any) => ({
      ...
      const recentStatements = (Array.isArray(statements) ? statements : []).map((statement: any) => ({
  ...
  action("listInsurancePlans", {
    async invoke() {
      const body = await inheritedOrFallback(
        `${PORTAL}/my-health/insurance-and-pharmacy`,
        /\/ema\/ws\/v3\/portal\/patient\/\d+\/insurance\/active(?:\?|$)/,
        "listInsurancePlans",
        async () => {
          const id = await patientId();
      ...
      const items = (Array.isArray(body) ? body : []).map((plan: any) => ({
        id: string(plan?.id),
  ...
  action("listPharmacies", {
    async invoke() {
      const body = await inheritedOrFallback(
        `${PORTAL}/my-health/insurance-and-pharmacy`,
        /\/ema\/ws\/v3\/portal\/patient\/\d+\/firm-user-pharmacies(?:\?|$)/,
        "listPharmacies",
        async () => {
          const id = await patientId();
      ...
      const items = (Array.isArray(body) ? body : []).map((pharmacy: any) => ({
