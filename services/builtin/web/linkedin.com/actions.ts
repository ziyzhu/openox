...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  const requireCsrf = (): string => {
    ...
    if (!t) throw new Error("Not signed in to LinkedIn (no JSESSIONID cookie). Sign in first.");
    return t.replace(/"/g, "");
  };
  ...
  const apiGet = async (path: string, accept: string): Promise<any> => {
  ...
  const gqlUrl = (gateway: string, queryId: string, variables: string): string =>
    `/voyager/api/${gateway}/graphql?queryId=${queryId}&variables=${variables}`;

  const enc = (urn: string): string => encodeURIComponent(urn);
  ...
  const threadIdOf = (convUrn: string): string => {
    const m = convUrn.match(/,([^,)]+)\)$/);
    if (!m) throw new Error("could not parse a thread id from the conversation URN");
    return m[1];
  };
  ...
  const idFromUrn = (urn: string | undefined | null): string => {
    const s = String(urn ?? "");
    const m = s.match(/[^:]+$/);
    return m ? m[0] : "";
  };
  ...
  const vectorUrl = (img: any, width: number): string => {
    const v = img?.displayImageReference?.vectorImage || img;
    const arts = v?.artifacts;
    if (!v?.rootUrl || !Array.isArray(arts) || !arts.length) return "";
    let best = arts[0];
    for (const a of arts) {
      if (Math.abs((a.width || 0) - width) < Math.abs((best.width || 0) - width)) best = a;
    }
    return v.rootUrl + (best.fileIdentifyingUrlPathSegment || "");
  };

  const profileVanity = (raw: string | undefined | null): string => {
    const s = String(raw ?? "").trim();
    const m = s.match(/\/in\/([^/?#]+)/);
    const v = (m ? m[1] : s).replace(/^@+/, "");
    if (!v) throw new Error("publicId is required (a LinkedIn vanity name or profile URL)");
    return v;
  };

  const year = (d: any): number | null => (d?.year ? Number(d.year) : null);

  const positionRow = (p: any): any => ({
    title: p?.title || "",
  ...
  const participantName = (p: any): string => {
    ...
    if (pt.organization) return pt.organization.name?.text || "";
    if (pt.agent) return pt.agent.title?.text || "";
    return "";
  };
  ...
  const nameMapOf = (participants: any[]): Record<string, string> => {
    const map: Record<string, string> = {};
    for (const p of participants || []) {
      const urn = p?.hostIdentityUrn;
      const name = participantName(p);
      if (urn && name) map[urn] = name;
    }
    return map;
  };

  const senderName = (m: any, nameMap: Record<string, string>): string =>
    participantName(m?.sender) || nameMap[m?.sender?.hostIdentityUrn || ""] || "";

  const messageText = (m: any): string =>
    m?.body?.text || m?.renderContentFallbackText || "";

  const messageRow = (m: any, nameMap: Record<string, string> = {}): any => ({
  ...
  const conversationRow = (c: any): any => {
  ...
  const includedByUrn = (included: any[]): Record<string, any> => {
    const map: Record<string, any> = {};
    for (const x of included || []) if (x?.entityUrn) map[x.entityUrn] = x;
    return map;
  };

  const eventSender = (from: any, byUrn: Record<string, any>): string => {
    if (!from) return "";
    const mp = from.miniProfile || byUrn[from["*miniProfile"] || ""];
    if (mp && (mp.firstName || mp.lastName)) {
      return `${mp.firstName || ""} ${mp.lastName || ""}`.trim();
    }
    if (from.alternateName) return from.alternateName;
    const mc = from.miniCompany || byUrn[from["*miniCompany"] || ""];
    return mc?.name || "";
  };

  const eventRow = (ev: any, byUrn: Record<string, any>): any => {
  ...
  const attr = (a: any): string => a?.text || "";

  // A search hit's kind is encoded in its tracking URN (urn:li:company:..,
  // urn:li:member:.., urn:li:group:.., urn:li:fsd_jobPosting:..).
  const searchType = (urn = ""): string => {
    if (/company/.test(urn)) return "company";
    if (/member|fsd_profile/.test(urn)) return "person";
    if (/\bgroup/.test(urn)) return "group";
    if (/job/.test(urn)) return "job";
    if (/school/.test(urn)) return "school";
    if (/course/i.test(urn)) return "course";
    return urn.split(":")[2] || "other";
  };

  const searchRow = (er: any): any => ({
  ...
  const jobIdOf = (urn = ""): string => (urn.match(/(\d+)\s*$/) || [, ""])[1] || "";

  const jobRow = (c: any): any => {
  ...
  const postRow = (u: any, byUrn: Record<string, any>): any => {
  ...
  const fetchMe = async (): Promise<any> => {
    ...
    const miniUrn: string = data["*miniProfile"] || "";
    const mini = (json?.included || []).find(
      (x: any) => x?.entityUrn === miniUrn || String(x?.$type || "").endsWith("MiniProfile"),
    );
    if (!mini) throw new Error("Could not resolve the signed-in member profile");
  ...
  action("getSignInState", {
    async invoke() {
      try {
        if (!cookie("JSESSIONID")) return { signedIn: false };
        const json = await apiGet("/voyager/api/me", NORMALIZED);
  ...
  action("getProfile", {
    async invoke({ publicId }: { publicId?: string } = {}) {
      const vanity = profileVanity(publicId);
      const json = await apiGet(
        `/voyager/api/identity/dash/profiles?q=memberIdentity&memberIdentity=${encodeURIComponent(vanity)}&decorationId=${PROFILE_DECO}`,
        NORMALIZED,
      );
      const included = json?.included || [];
      const profile = included.find((x: any) => String(x?.$type || "").endsWith("profile.Profile"));
      if (!profile) throw new Error(`LinkedIn member @${vanity} not found`);
      const geoUrn = profile.geoLocation?.geoUrn;
      const geo = geoUrn && included.find((x: any) => x?.entityUrn === geoUrn);
      const location = profile.locationName || geo?.defaultLocalizedName || "";
      const positions = included
        .filter((x: any) => String(x?.$type || "").endsWith("profile.Position"))
        .map(positionRow)
        .sort((a: any, b: any) => (b.startYear || 0) - (a.startYear || 0));
      const educations = included
        .filter((x: any) => String(x?.$type || "").endsWith("profile.Education"))
        .map(educationRow)
        .sort((a: any, b: any) => (b.startYear || 0) - (a.startYear || 0));
      return {
  ...
  action("listConversations", {
    async invoke() {
      ...
      const json = await apiGet(
        gqlUrl("voyagerMessagingGraphQL", QUERY.conversations, `(mailboxUrn:${enc(mailboxUrn)})`),
        GRAPHQL,
      );
  ...
  action("getConversation", {
    async invoke({ id, cursor, limit }: { id?: string; cursor?: string; limit?: number } = {}) {
      ...
      if (cursor) qs.set("createdBefore", String(Number(cursor)));
      const json = await apiGet(
        `/voyager/api/messaging/conversations/${enc(threadIdOf(convUrn))}/events?${qs.toString()}`,
        NORMALIZED,
      );
      const byUrn = includedByUrn(json?.included);
      const urns: string[] = json?.data?.["*elements"] || [];
      const elements = urns
        .map((u) => byUrn[u])
        .filter(Boolean)
        .sort((a: any, b: any) => (Number(a?.createdAt) || 0) - (Number(b?.createdAt) || 0));
      const items = elements.map((ev: any) => eventRow(ev, byUrn));
  ...
  action("search", {
    async invoke({ query, cursor, limit }: { query?: string; cursor?: string; limit?: number } = {}) {
      const q = String(query ?? "").trim();
      if (!q) throw new Error("query is required");
      const count = Math.min(Math.max(Number(limit) || 10, 1), 25);
      const start = cursor ? Number(cursor) : 0;
      const json = await apiGet(
        `/voyager/api/search/dash/clusters?decorationId=${SEARCH_DECO}&origin=GLOBAL_SEARCH_HEADER&q=all` +
          `&query=(keywords:${encodeURIComponent(q)},flagshipSearchIntent:SEARCH_SRP)&start=${start}&count=${count}`,
        NORMALIZED,
      );
      const items = (json?.included || [])
        .filter((x: any) => String(x?.$type || "").endsWith("EntityResultViewModel"))
        .map(searchRow)
  ...
  action("searchJobs", {
    async invoke({ query, location, cursor, limit }: { query?: string; location?: string; cursor?: string; limit?: number } = {}) {
      const q = String(query ?? "").trim();
      if (!q) throw new Error("query is required");
      const count = Math.min(Math.max(Number(limit) || 10, 1), 25);
      const start = cursor ? Number(cursor) : 0;
      ...
      if (loc) queryParts.push(`locationFallback:${encodeURIComponent(loc)}`);
      const json = await apiGet(
        `/voyager/api/voyagerJobsDashJobCards?decorationId=${JOBS_DECO}&q=jobSearch` +
          `&query=(${queryParts.join(",")})&start=${start}&count=${count}`,
        NORMALIZED,
      );
      const items = (json?.included || [])
        .filter((x: any) => String(x?.$type || "").endsWith("JobPostingCard"))
        .map(jobRow)
        .filter((r: any) => r.title && r.jobId);
  ...
  action("listFeed", {
    async invoke({ cursor, limit }: { cursor?: string; limit?: number } = {}) {
      ...
      const start = cursor ? Number(cursor) : 0;
      const json = await apiGet(
        `/voyager/api/feed/updatesV2?commentsCount=0&count=${count}&q=chronFeed&start=${start}`,
        NORMALIZED,
      );
      const included = json?.included || [];
      const byUrn = includedByUrn(included);
      const items = included
        .filter((x: any) => String(x?.$type || "").endsWith("render.UpdateV2"))
        .map((u: any) => postRow(u, byUrn))
        .filter((r: any) => r.author || r.text);
      return { items, nextCursor: items.length ? String(start + count) : null };
    },
  });
};
