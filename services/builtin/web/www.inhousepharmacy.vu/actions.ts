...
const install: ActionInstaller = ({ action, retryFetch, log }) => {
  ...
  const productIdFrom = (value: string | null | undefined) =>
    cleanText(value).match(/\/p-(\d+)-/i)?.[1] ?? null;

  const fetchDoc = async (path: string) => {
    const url = path.startsWith("http") ? path : BASE + path;
    const response = await retryFetch(url, { credentials: "include" });
    if (!response.ok) throw new Error(`Inhouse Pharmacy returned HTTP ${response.status}.`);
    const doc = new DOMParser().parseFromString(await response.text(), "text/html");
    return { doc, url: response.url };
  };

  const productUrl = (input: string) => {
  ...
  const valueAfterBold = (root: ParentNode, label: string) => {
    for (const bold of root.querySelectorAll("b")) {
      if (cleanText(bold.textContent).replace(/:$/, "").toLowerCase() !== label.toLowerCase()) continue;
      return cleanText(bold.nextSibling?.textContent);
    }
    return "";
  };

  const detailValue = (doc: Document, label: string) => {
    for (const detail of doc.querySelectorAll(".pdetail")) {
      ...
      const value = detail.cloneNode(true) as Element;
      value.querySelector(".leftPName1")?.remove();
      for (const br of value.querySelectorAll("br")) br.replaceWith(" ");
      return cleanText(value.textContent);
    }
    return "";
  };

  const parseCart = (doc: Document) => {
  ...
  const receiptValue = (doc: Document, label: string) => {
  ...
  const receiptTotal = (doc: Document) => {
    for (const row of doc.querySelectorAll("#tblOrderSummary tr")) {
      ...
      if (label.toLowerCase() === "total") return cleanText(row.querySelector(".tdOrderSummaryValue")?.textContent);
    }
    return "";
  };
  ...
  action("searchProducts", {
    async invoke({ query }) {
      ...
      if (search.length < 3) throw new Error("Search with at least three characters.");
  ...
  action("getProduct", {
    async invoke({ url }) {
      ...
      if (!id || !name) throw new Error("The product page did not contain product details.");
  ...
  action("addCartItem", {
    async invoke({ url, variantId, quantity = 1 }) {
      ...
      if (!form) throw new Error("The product form is unavailable.");
      const body = new URLSearchParams();
      for (const input of form.querySelectorAll("input[name]")) {
        const type = cleanText(input.getAttribute("type")).toLowerCase();
        if (["button", "submit", "image", "file", "password"].includes(type)) continue;
        if (["checkbox", "radio"].includes(type) && !input.hasAttribute("checked")) continue;
      ...
      if (!response.ok) throw new Error(`Cart update returned HTTP ${response.status}.`);
      await response.text();
      const cart = await fetchCart();
  ...
  action("getOrder", {
    async invoke({ id }) {
      ...
      if (!/^\d+$/.test(normalizedId)) throw new Error("Use an order ID from the order list.");
      const orders = await fetchOrders();
      const summary = orders.find((order) => order.id === normalizedId);
      if (!summary) throw new Error(`Order ${id} was not found in the signed-in account.`);
      ...
      if (!items.length) throw new Error(`Order ${id} did not contain receipt items.`);
