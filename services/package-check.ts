...
try {
  ...
  const [packagedManifest, bundledManifest] = await Promise.all([
    readFile(join(import.meta.dir, "dist", "repository", "ox.json"), "utf8"),
    readFile(resolve(import.meta.dir, "../ios/ios/OxServices.bundle/ox.json"), "utf8"),
  ]);
  if (packagedManifest !== bundledManifest) throw new Error("npm repository differs from ios/ios/OxServices.bundle");
  ...
  const unexpected = files.filter((path) => !["LICENSE", "README.md", "package.json", "repository.ts"].includes(path)
    && !path.startsWith("dist/repository/"));
  if (unexpected.length) throw new Error(`unexpected package files: ${unexpected.join(", ")}`);
  if (!files.includes("dist/repository/ox.json")) throw new Error("package does not contain dist/repository/ox.json");
  ...
  await Bun.write(checkPath, [
    ...
    'const indexPath = fileURLToPath(import.meta.resolve("@openox/services/ox.json"));',
    'const manifestPath = fileURLToPath(import.meta.resolve("@openox/services/web/github.com/manifest.json"));',
    'if (!(await Bun.file(indexPath).exists())) throw new Error("ox.json export failed");',
  ...
  const raw = JSON.parse(await readFile(join(repositoryRoot, "ox.json"), "utf8"));
