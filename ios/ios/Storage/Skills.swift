...
nonisolated enum SkillFiles {
    ...
    static func migrateLegacy(at root: URL) throws -> Int {
        ...
        var existing = Set(try fm.contentsOfDirectory(atPath: skillsRoot.path))
        var claimed: Set<String> = []
        var migrated = 0
        for legacySkill in parseLegacy(text) {
            let base = slug(legacySkill.name)
            ...
            while true {
                ...
                let skill = Skill(name: name, description: description, instructions: legacySkill.instructions, services: legacySkill.services)
                ...
                if existing.contains(name) {
                    guard let current = try? String(contentsOf: file, encoding: .utf8),
                          parse(current, directoryName: name) == skill else { continue }
                } else {
    ...
    static func migrateUserNamespace(at root: URL) throws -> Int {
        let namespace = "profile"
        let skillsRoot = root.appendingPathComponent("skills", isDirectory: true)
        let fm = FileManager.default
        guard fm.fileExists(atPath: skillsRoot.path) else { return 0 }
        ...
        for source in directories {
            guard (try source.resourceValues(forKeys: [.isDirectoryKey])).isDirectory == true,
                  ...
                  let localName = legacyLocalName(source.lastPathComponent, namespace: namespace),
                  let text = try? String(
                    contentsOf: source.appendingPathComponent(fileName),
                    encoding: .utf8
                  ),
                  let skill = parse(text, directoryName: source.lastPathComponent) else { continue }
            ...
            if fm.fileExists(atPath: destination.path) {
                guard let existing = try? String(contentsOf: destinationFile, encoding: .utf8),
                      parse(existing, directoryName: destinationName) == migratedSkill else {
                ...
                try serialize(migratedSkill).write(
                    to: staging.appendingPathComponent(fileName),
                    atomically: true,
                    encoding: .utf8
                )
    ...
    private static func legacyLocalName(_ name: String, namespace: String) -> String? {
        if isLocalName(name) { return name }
        ...
        guard parts.count == 2,
              ...
              isLocalName(parts[0]),
              isLocalName(parts[1]) else { return nil }
    ...
    private static func parseLegacy(_ text: String) -> [Skill] {
        var skills: [Skill] = []
        var name: String?
        var services: [String] = []
        var body: [Substring] = []
        func flush() {
            ...
            skills.append(Skill(name: name, description: instructions, instructions: instructions, services: services))
        }
