...
actor ServiceRepository {
    ...
    func monoRepository() async throws -> MonoRepository {
        ...
        for candidate in selected {
            ...
            guard let data = try? Data(contentsOf: serviceRoot.appendingPathComponent("manifest.json")) else { continue }
    ...
    private func createWebService(domain: String) throws {
        ...
        do {
            ...
            try manifestData.write(to: serviceRoot.appendingPathComponent("manifest.json"), options: .atomic)
            try Data("(() => { window.ox = { async callServiceAction(name) { throw new Error(`unknown action: ${name}`); } }; })();\n".utf8)
    ...
    func listSource(kind: ServiceKind, id: String, path: [String]) throws -> [Entry] {
        ...
        if source.provenance != .local {
            guard path.isEmpty else { throw Failure(message: "Only Local service source is available beyond manifest.json.") }
            let manifest = source.root.appendingPathComponent("manifest.json")
            let size = try manifest.resourceValues(forKeys: [.fileSizeKey]).fileSize
            return [Entry(name: "manifest.json", isDirectory: false, size: size)]
        }
        ...
        return try FileManager.default.contentsOfDirectory(
            ...
            if values.isRegularFile == true { return Entry(name: url.lastPathComponent, isDirectory: false, size: values.fileSize) }
            return nil
    ...
    func readSource(kind: ServiceKind, id: String, path: [String]) throws -> Data {
        let source = try activeSource(kind: kind, id: id)
        guard source.provenance == .local || path == ["manifest.json"] else {
            throw Failure(message: "Only Local service source is available beyond manifest.json.")
        }
    ...
    func sourcePaths(kind: ServiceKind, id: String) throws -> [String] {
        let source = try activeSource(kind: kind, id: id)
        if source.provenance != .local { return ["services/\(kind.rawValue)/\(id)/manifest.json"] }
        ...
        while let url = enumerator.nextObject() as? URL {
            ...
            let relative = url.path.replacingOccurrences(of: source.root.path + "/", with: "", options: [.anchored])
    ...
    private func materializeLocalRepository() throws {
        let packageURL = localRoot.appendingPathComponent("ox.json")
    ...
    private func sourceURL(_ source: ActiveSource, path: [String]) throws -> URL {
        guard path.allSatisfy({
            !$0.isEmpty && $0 != "." && $0 != ".." && !$0.hasPrefix(".") && !$0.contains("/") && !$0.contains("\\")
        }) else {
            throw Failure(message: "Invalid service path")
        }
        let url = path.reduce(source.root) { $0.appendingPathComponent($1) }.standardizedFileURL
    ...
    private static func writePackage(_ package: Package, at root: URL) throws {
        ...
        data.append(0x0A)
        try data.write(to: root.appendingPathComponent("ox.json"), options: .atomic)
    }
    ...
    private static func loadPackage(at root: URL, provenance: Repository.Provenance) throws -> Package {
        let packageURL = root.appendingPathComponent("ox.json")
        ...
        guard values.isSymbolicLink != true, (values.fileSize ?? 0) <= 512_000 else {
            throw Failure(message: "ox.json is invalid or too large")
        }
        let data = try Data(contentsOf: packageURL)
        try validatePackageShape(data)
        guard let package = try? JSONDecoder().decode(Package.self, from: data) else {
            throw Failure(message: "ox.json is invalid")
        }
        guard package.version == 1 else { throw Failure(message: "Unsupported ox.json version") }
        guard !package.name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
            ...
            throw Failure(message: "ox.json metadata is invalid")
        }
        if let hash = package.contentHash,
           hash.range(of: "^[a-f0-9]{64}$", options: .regularExpression) == nil {
            throw Failure(message: "ox.json content hash is invalid")
        }
    ...
    private static func validateService(_ service: Package.Service, root: URL) throws {
        ...
        let manifestURL = serviceRoot.appendingPathComponent("manifest.json")
    ...
    private static func validatePackageShape(_ data: Data) throws {
        ...
        else { throw Failure(message: "ox.json is invalid") }
    }
    ...
    private static func package(at commit: Commit, in repository: SwiftGitX.Repository) throws -> Package {
        let blob = try blob(at: "ox.json", commit: commit, repository: repository)
