(() => {
  // services/builtin/web/creator.xiaohongshu.com/actions.ts
  var install = ({ action, retryFetch, log }) => {
    ...
    const captureData = async (pattern, label, replayLatest) => {
      const capture = window.oxFetchCapture;
      if (!capture)
        throw new Error("response capture is unavailable");
      ...
      if (!response || response.success !== true || number(response.code) !== 0) {
        throw new Error(`${label}: ${response?.msg || response?.result || "request failed"}`);
      }
    ...
    const nextPageButton = () => document.querySelector(".el-pagination button.btn-next:not([disabled]), .el-pagination .btn-next:not(.is-disabled)");
    ...
    const noteUrl = (note) => {
      ...
      if (note?.xsec_token)
        params.set("xsec_token", String(note.xsec_token));
      if (note?.xsec_source)
        params.set("xsec_source", String(note.xsec_source));
      const query = params.toString();
      return `${PUBLIC_ORIGIN}/explore/${encodeURIComponent(id)}${query ? `?${query}` : ""}`;
    };
    const creatorNote = (note) => {
      const images = Array.isArray(note?.images_list) ? note.images_list.map((image) => String(image?.url ?? "")).filter(Boolean) : [];
      return {
        id: String(note?.id ?? ""),
    ...
    const noteCard = async (id) => waitFor(() => Array.from(document.querySelectorAll(".note-card")).find((card) => String(card.getAttribute("data-impression") ?? "").includes(id)));
    const dialog = async (text) => waitFor(() => Array.from(document.querySelectorAll("[role='dialog'], .d-modal, .el-dialog")).filter(visible).find((element) => text.test(element.textContent ?? "")));
    const confirmDialog = (container) => {
      const button = Array.from(container.querySelectorAll("button")).filter(visible).find((candidate) => /^(确定|确认)$/.test(candidate.textContent?.trim() ?? ""));
      if (!(button instanceof HTMLElement))
        throw new Error("confirmation control unavailable");
      button.click();
    };
    const captureMutation = async (pattern, label) => {
      const capture = window.oxFetchCapture;
      if (!capture)
        throw new Error("response capture is unavailable");
      ...
      const success = response?.success === true || response && "code" in response && number(response.code) === 0 || response?.result?.success === true;
      if (!response || !success) {
        throw new Error(`${label}: ${response?.msg || response?.result?.message || "request failed"}`);
      }
    ...
    const currentUserId = () => String(document.querySelector("#app")?.__vue_app__?.config?.globalProperties?.$store?.state?.Auth?.userInfo?.userId ?? window.publish?.getUserState?.()?.userInfo?.value?.id ?? "");
    const draftTitle = (content) => String(content?.articleTitle ?? content?.draftStore?.title ?? content?.title ?? "");
    const draftDescription = (content) => String(content?.shortTextContent ?? content?.draftStore?.desc ?? content?.description ?? content?.desc ?? "");
    const draftCover = (content) => {
      const cover = content?.draftStore?.coverUrl ?? content?.draftStore?.cover ?? content?.coverUrl ?? "";
      return typeof cover === "string" ? cover : String(cover?.url ?? cover?.src ?? "");
    };
    const paginationNumber = (element) => element.querySelector(".d-pagination-page-content")?.textContent?.trim() ?? "";
    action("getSignInUrl", {
      async invoke() {
    ...
    action("getSignInState", {
      async invoke() {
        const response = await retryFetch(`${ORIGIN}/api/galaxy/creator/home/personal_info`, {
          credentials: "include",
        ...
        const signedIn = value?.success === true && number(value?.code) === 0 && value?.data !== null && typeof value?.data === "object";
    ...
    action("getCreatorProfile", {
      async invoke() {
        const data = await captureData(/\/api\/galaxy\/creator\/home\/personal_info(?:\?|$)/, "creator profile", true);
        markPageUsed();
        return {
    ...
    action("getCreatorAnalytics", {
      async invoke() {
        const data = await captureData(/\/api\/galaxy\/v2\/creator\/datacenter\/account\/base(?:\?|$)/, "creator analytics", true);
        markPageUsed();
        return {
    ...
    action("listCreatorNotes", {
      async invoke({ cursor } = {}) {
        ...
        let data = await captureData(/\/api\/galaxy\/v2\/creator\/note\/user\/posted(?:\?|$)/, "creator notes", true);
        for (let index = 0;index < page; index++) {
          const button = await waitFor(nextPageButton);
          const pending = captureData(/\/api\/galaxy\/v2\/creator\/note\/user\/posted(?:\?|$)/, "creator notes", false);
          button.click();
          data = await pending;
        }
        markPageUsed();
        const items = Array.isArray(data.notes) ? data.notes.map(creatorNote) : [];
        const nextPage = Number(data.page);
        return {
          items,
          nextCursor: Number.isInteger(nextPage) && nextPage >= 0 ? String(nextPage) : null
        };
      }
    });
    action("listCreatorActivities", {
      async invoke() {
        const data = await captureData(/\/api\/galaxy\/v2\/creator\/activity_center\/list(?:\?|$)/, "creator activities", true);
        markPageUsed();
        return {
          items: (Array.isArray(data.activity_list) ? data.activity_list : []).map((item) => ({
            ...
            topics: (Array.isArray(item.topic_infos) ? item.topic_infos : []).map((topic) => ({
              id: String(topic.id ?? ""),
    ...
    action("listCreatorInspiration", {
      async invoke() {
        const data = await captureData(/\/api\/galaxy\/creator\/select\/topic\/detail(?:\?|$)/, "creator inspiration", true);
        markPageUsed();
        return {
          groups: (Array.isArray(data) ? data : []).map((group) => ({
            id: String(group.labelId ?? ""),
            name: String(group.labelName ?? ""),
            topics: (Array.isArray(group.selectTopics) ? group.selectTopics : []).map((topic) => ({
              id: String(topic.id ?? ""),
              title: String(topic.title ?? ""),
              url: String(topic.link ?? ""),
              participantCount: number(topic.joinNum),
              viewCount: number(topic.viewNum),
              exampleNotes: (Array.isArray(topic.notes) ? topic.notes : []).map((note) => ({
                id: String(note.noteId ?? ""),
                title: String(note.title ?? ""),
                type: String(note.type ?? ""),
                likes: number(note.likes),
                images: (Array.isArray(note.images_list) ? note.images_list : []).map((image) => String(image?.url ?? image ?? "")).filter(Boolean)
              }))
            }))
          }))
        };
    ...
    action("listCreationGuidance", {
      async invoke({
        ...
        if (!selected)
          throw new Error(`invalid guidance category: ${category}`);
        ...
        const panels = await waitFor(() => {
          ...
          return items.length >= 2 && items.every((item) => item.querySelector(".card")) ? items : null;
        });
        const panel = panels[selected.type === 1 ? 0 : 1];
        if (!(panel instanceof HTMLElement))
          throw new Error("creation guidance panel unavailable");
        const categoryButton = Array.from(panel.querySelectorAll(".button-group-item")).find((element) => element.textContent?.trim() === selected.label);
        if (selected.type !== 1 && categoryButton && !categoryButton.classList.contains("active")) {
          const pending = captureData(pattern, "creation guidance", false);
          categoryButton.click();
          data = await pending;
        } else if (selected.type === 1 && page === 1) {
          const secondPage = Array.from(panel.querySelectorAll(".d-pagination-page")).find((element) => paginationNumber(element) === "2");
          if (!secondPage)
            throw new Error("official guidance pagination unavailable");
          const next = captureData(pattern, "creation guidance", false);
          secondPage.click();
          await next;
          const firstPage = await waitFor(() => Array.from(panel.querySelectorAll(".d-pagination-page")).find((element) => paginationNumber(element) === "1"));
          const pending = captureData(pattern, "creation guidance", false);
          firstPage.click();
          data = await pending;
        } else {
        ...
        if (page > 1) {
          const pageButton = await waitFor(() => Array.from(panel.querySelectorAll(".d-pagination-page")).find((element) => paginationNumber(element) === String(page)));
        ...
        return {
          items: (Array.isArray(data.create_guidance) ? data.create_guidance : []).map((item) => ({
    ...
    action("listCreatorNotices", {
      async invoke() {
        const data = await captureData(/\/api\/galaxy\/v2\/creator\/notice(?:\?|$)/, "creator notices", true);
        markPageUsed();
        return {
          items: (Array.isArray(data.notice_list) ? data.notice_list : []).map((item) => ({
            title: String(item.title ?? ""),
            url: String(item.link ?? ""),
    ...
    action("getLiveAnalytics", {
      async invoke() {
        const data = await captureData(/\/api\/galaxy\/v2\/creator\/datacenter\/livedata\/overview(?:\?|$)/, "live analytics", true);
    ...
    action("getNoteAnalytics", {
      async invoke({ noteId }) {
        ...
        const [base, trend] = await Promise.all([
          captureData(/\/api\/galaxy\/creator\/datacenter\/note\/base(?:\?|$)/, "note analytics", true),
          captureData(/\/api\/galaxy\/creator\/datacenter\/note\/analyze\/audience\/trend(?:\?|$)/, "note audience trend", true)
        ]);
        ...
        return {
          note: {
            ...
            tags: (Array.isArray(note.tags) ? note.tags : []).map((tag) => ({
              id: String(tag.id ?? ""),
          ...
          images: (Array.isArray(trend.images_list) ? trend.images_list : []).map((image) => String(image?.url ?? "")).filter(Boolean),
    ...
    action("listCreatorDrafts", {
      async invoke() {
        ...
        if (databases.length && !databases.some((database2) => database2.name === "draft-database-v1")) {
          return { items: [] };
        }
        ...
        database.close();
        const items = records.flatMap(({ store, items: values }) => values.filter((record) => String(record?.uid ?? "") === userId).map((record) => ({
    ...
    action("updateNoteVisibility", {
      async invoke({
        ...
        const button = card.querySelectorAll(".note-card__action-btn")[0];
        if (!button)
          throw new Error("note visibility control unavailable");
        button.click();
        const modal = await dialog(/可见|公开|私密/);
        const select = modal.querySelector(".d-select");
        if (!select)
          throw new Error("visibility menu unavailable");
        select.click();
        const option = await waitFor(() => Array.from(document.querySelectorAll(".d-popover .custom-option")).filter(visible).find((element) => element.textContent?.trim() === label));
        if (!(option instanceof HTMLElement))
          throw new Error(`visibility option unavailable: ${label}`);
        option.click();
        const pending = captureMutation(/\/web_api\/sns\/v1\/note\/privacy(?:\?|$)/, "note visibility");
        confirmDialog(modal);
        await pending;
        markPageUsed();
    ...
    action("updatePinnedNote", {
      async invoke({ noteId, pinned }) {
        ...
        const button = card.querySelectorAll(".note-card__action-btn")[1];
        if (!button)
          throw new Error("note pin control unavailable");
        button.click();
        const modal = await dialog(/置顶|取消置顶/);
        const pending = captureMutation(/\/api\/galaxy\/creator\/sns\/note\/top(?:\?|$)/, "note pin");
        confirmDialog(modal);
        await pending;
        markPageUsed();
        return { noteId, pinned };
    ...
    action("deleteCreatorNote", {
      async invoke({ noteId }) {
        if (!noteId?.trim())
          throw new Error("noteId is required");
        const card = await noteCard(noteId);
        const button = card.querySelector(".note-card__action-btn--del");
        if (!button)
          throw new Error("note delete control unavailable");
        button.click();
        const modal = await dialog(/删除后|删除笔记|无法恢复/);
        const pending = captureMutation(/\/web_api\/sns\/capa\/postgw\/note\/delete(?:\?|$)/, "note deletion");
        confirmDialog(modal);
        await pending;
        markPageUsed();
  ...
  installService("creator.xiaohongshu.com", actions_default);
})();
