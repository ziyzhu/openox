(() => {
  // services/builtin/web/ctrip.com/actions.ts
  var install = ({ action, log }) => {
    ...
    const flightPageState = () => {
      dismissFlightModal();
      if (/captcha/.test(location.pathname) || /验证码|verify the human|拖动滑块/i.test(document.body?.innerText || ""))
        return "captcha";
    ...
    const scrapeFlights = () => {
      ...
      document.querySelectorAll(".flight-list > span > div").forEach((card) => {
        ...
        items.push({
          ...
          flightNo,
          aircraft,
          departTime: chunks[firstTimeIdx],
          departAirport: depAirport,
          arriveTime: chunks[arrTimeIdx],
    ...
    action("searchFlightCities", {
      async invoke({ query }) {
        try {
          ...
          const seen = new Set;
          const items = cities.filter((c) => !q || [c.name, c.nameEn, c.namePy, c.code].some((s) => String(s ?? "").toLowerCase().includes(q))).filter((c) => c.code && !seen.has(c.code) && seen.add(c.code)).map((c) => ({ code: c.code, name: c.name, nameEn: c.nameEn ?? "" }));
      ...
      async invoke({ fromCity, toCity, startDate, returnDate }) {
        try {
          const j = await postJSON(`${API}/15380/bjjson/FlightIntlAndInlandLowestPriceSearch`, {
            departNewCityCode: fromCity,
            arriveNewCityCode: toCity,
            searchType: 1,
            flag: 4,
        ...
        try {
          ...
          const walk = (o) => {
            ...
            if (o.displayCityModel && o.basicCityModel) {
              const { displayCityModel: d, basicCityModel: b } = o;
              cities.push({
                cityId: b.cityId,
          ...
          const q = String(query ?? "").trim().toLowerCase();
          const seen = new Set;
          const items = cities.filter((c) => !q || [c.name, c.nameEN].some((s) => String(s ?? "").toLowerCase().includes(q))).filter((c) => c.cityId && !seen.has(c.cityId) && seen.add(c.cityId));
          return { items, nextCursor: null };
        } catch (e) {
  ...
  installService("ctrip.com", actions_default);
})();
