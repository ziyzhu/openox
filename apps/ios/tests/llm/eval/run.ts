...
function usage(): string {
  return `Usage: bun apps/ios/tests/llm/eval/run.ts --target <provider=client:model> --target <provider=client:model> [options]
...
Options:
  ...
  --cases <path>                   Corpus path (default apps/ios/tests/llm/cases/acceptance.json)
...
Examples:
  bun apps/ios/tests/llm/eval/run.ts --target openai=chatgpt:gpt-5.6-sol --target google=gemini:gemini-3.6-flash
  bun apps/ios/tests/llm/eval/run.ts --target openai=chatgpt:gpt-5.6-sol --allow-single-provider --group execute`;
}
...
function parseOptions(args: string[]): Options | undefined {
  ...
  const options: Options = {
    targets: [],
    casesPath: join(ROOT, "apps/ios/tests/llm/cases/acceptance.json"),
    caseIds: [],
    groups: [],
    chat: "",
    runs: 3,
    ...
    output: join(ROOT, `apps/ios/build/llm-evals/${stamp}.json`),
