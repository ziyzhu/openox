...
import { debugEndpoint } from "../../../../cli/src/debug-ws.ts";
...
function usage(): string {
  return `Usage: bun apps/ios/tests/llm/benchmark/run.ts --target <provider=client:model> [options]
...
Options:
  ...
  --cases <path>                   Eval corpus path (default apps/ios/tests/llm/cases/acceptance.json)
...
Example:
  OX_DEBUG_ENDPOINT=ws://127.0.0.1:9101 bun apps/ios/tests/llm/benchmark/run.ts --target openai=chatgpt:gpt-5.6-sol`;
}
...
function parseOptions(args: string[]): Options | undefined {
  ...
  const options: Options = {
    targets: [],
    caseIds: [],
    casesPath: resolve(ROOT, "apps/ios/tests/llm/cases/acceptance.json"),
    chat: "",
    runs: 5,
    warmups: 1,
    timeoutMs: 120_000,
    minimumPassRate: 0.8,
    output: resolve(ROOT, `apps/ios/build/llm-benchmarks/${stamp}.json`),
    baseline: "",
  };
