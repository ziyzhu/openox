window.ox.install(1, ({ action, retryFetch, log, lib }) => {
    ...
    const recordUrl = (db, uid) => db === "pubmed"
    ...
    const authorsOf = (summary) => Array.isArray(summary?.authors)
        ? summary.authors.map((author) => author?.name).filter((name) => typeof name === "string")
        : undefined;
    ...
    const sourceOf = (summary) => summary?.fulljournalname || summary?.source || summary?.book
    ...
    const articleIdOf = (summary, type) => Array.isArray(summary?.articleids)
        ? summary.articleids.find((articleId) => articleId?.idtype === type)?.value
        : undefined;
