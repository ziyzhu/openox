window.ox.install(1, ({ action, retryFetch, log, lib }) => {
    ...
    const mapPaymentFilterAccount = (account) => ({
        ...
        activityTypes: Array.isArray(account?.activityTypes)
            ? account.activityTypes.map((value) => String(value))
            : [],
    });
    action("getPaymentActivityFilters", {
        async invoke() {
            ...
            const fromAccounts = Array.isArray(data?.fromAccounts)
                ? data.fromAccounts.map(mapPaymentFilterAccount).filter((account) => account.identifier)
                : [];
    ...
    action("listPaymentActivities", {
        async invoke({ cursor, limit, timeframe = "DEFAULTDAYS", fromDate, toDate, fromAccountIdentifiers, toAccountIdentifiers, keyword, minimumAmount, maximumAmount, activityTypes, statuses, } = {}) {
