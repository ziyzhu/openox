...
- `AgentConfiguration` owns execution configuration. `run(AgentRunRequest)` rejects busy submissions and returns an `AgentRunResult` with a completed, aborted, or failed outcome. Caller cancellation aborts only that accepted run.
- Initial context and configuration are captured at admission. Hooks and tool scheduling remain run-local; provider, model, prompt, tools, stream options, and compaction threshold refresh at generation boundaries, preserving live model switching.
- One run contains one or more generations, each comprising an assistant response and its tool results. `runFinished` is published after final context and idle state are installed. Runtime completion does not imply that a chat has consumed buffered events or finished presentation delivery; that settlement remains chat-owned.
