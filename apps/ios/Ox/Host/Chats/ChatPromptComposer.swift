...
enum ChatPromptComposer {
    ...
    private static func operatingRulesSection() -> String {
        ...
        - When a concrete next request follows from the current conversation, use `ox.user.follow` during this task to offer up to two short intents scoped to this chat. Do not suggest unrelated tasks from other chats or global memory. This does not pause the current task. Use a send intent only when it is complete without further input; tapping it fills the composer for the user to review and send.
