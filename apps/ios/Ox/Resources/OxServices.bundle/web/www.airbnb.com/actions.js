...
const numericId = (value) => graphIdPart(value) ?? value.match(/\d+/)?.[0] ?? value;
const stayId = (value) => {
...
const scalarText = (value) => cleanText(typeof value === "string" ? value : "");
const ratingParts = (value) => {
...
const rawParam = (filterName, value) => ({
    filterName,
    filterValues: [String(value)],
});
const numberFromItems = (items, label) => {
...
window.ox.install(1, ({ action, retryFetch, log, lib }) => {
    ...
    const tripRow = (trip) => {
        ...
        const travelerNames = [...new Set((trip?.travelers?.edges ?? [])
                .map((edge) => scalarText(edge?.node?.user?.displayFirstName))
                .filter(Boolean))];
        return {
            id: String(trip?.id ?? ""),
    ...
    action("searchStays", {
        async invoke({ query, checkIn, checkOut, adults = 1, children = 0, infants = 0, pets = 0, minBathrooms, freeParking = false, maxPrice, cursor, limit = 18, } = {}) {
    ...
    action("getTrip", {
        async invoke({ id } = {}) {
            ...
            const guests = (reservation?.guests?.edges ?? [])
                .map((edge) => scalarText(edge?.node?.guestUser?.presentation?.displayFirstName))
                .filter(Boolean);
            const output = {
                id: String(trip.id),
                listingId: supply?.id ? numericId(String(supply.id)) : null,
