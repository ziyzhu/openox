...
const articleCard = (element, fallbackJournal = null) => {
...
window.ox.install(1, ({ action, retryFetch, log, lib }) => {
    ...
    const metas = (doc, name) => [...doc.querySelectorAll(`meta[name="${name}"]`)]
    ...
    action("searchArticles", {
        async invoke({ query, author, title, journal, articleType, startYear, endYear, order = "relevance", cursor, }) {
            if (!query.trim())
    ...
    action("getArticle", {
        async invoke({ id }) {
            ...
            const sections = [...(body?.querySelectorAll("section[data-title]") ?? [])]
                .map((section) => {
    ...
    action("listJournals", {
        async invoke() {
            ...
            const items = [...doc.querySelectorAll('#journals-az li a[href^="/"]')]
                .map((link) => {
    ...
    action("getJournal", {
        async invoke({ id }) {
            ...
            const name = doc.querySelector('meta[property="og:title"]')?.content.trim()
            ...
            const groups = [...doc.querySelectorAll('section[data-track-component$=" grid"]')]
                .map((section) => {
