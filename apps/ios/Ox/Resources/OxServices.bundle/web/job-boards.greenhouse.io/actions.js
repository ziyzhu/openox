window.ox.install(1, ({ action, retryFetch, log, lib }) => {
    ...
    const loaderData = (document) => {
        const script = [...document.scripts].find((candidate) => candidate.textContent?.trimStart().startsWith("window.__remixContext = "));
    ...
    action("getJob", {
        async invoke({ board, id }) {
            ...
            const document = await fetchDocument(`https://job-boards.greenhouse.io/${encodeURIComponent(boardName)}/jobs/${encodeURIComponent(normalizedId)}`);
