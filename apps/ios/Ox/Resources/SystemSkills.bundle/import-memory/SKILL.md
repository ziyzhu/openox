...
Find the source app's bundled service with `ox.service.find`. Read the candidate's manifest, attach it with `ox.service.attach`, and inspect its actions before invoking them. Check `getSignInState` when available; account memory requires a confirmed signed-in state. If the probe reports signed out or errors, offer `ox.service.signIn` for that app or report the limitation rather than treating a guest chat as personal memory. Use available memory, conversation search, or conversation read actions to gather relevant context directly. If the service supports chatting, ask the source assistant to summarize what it can access:
