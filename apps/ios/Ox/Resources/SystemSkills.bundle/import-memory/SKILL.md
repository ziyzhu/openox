...
A chat action may create a conversation in the source app and require Ox approval. If its send outcome is uncertain, read the current conversation before considering any retry; do not send the prompt twice. Do not treat the source assistant's answer as a complete account history. Use read actions for relevant saved conversations when they add useful evidence; avoid a broad history import. If the service is unavailable or cannot expose useful context, explain the limitation and offer user-supplied notes as an optional fallback. Never ask the user to paste material that the service can retrieve, or request account credentials or a full export.
