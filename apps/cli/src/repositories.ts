...
export async function readRepository(root: string): Promise<RepositoryPackage> {
  const packagePath = existsSync(join(root, "repository.json"))
  ...
  for (const service of repository.services) {
    ...
    const manifestPath = existsSync(join(path, "service.json"))
...
export async function createSnapshot(sourceRoot: string): Promise<Snapshot> {
  ...
  try {
    ...
    for (const service of repository.services) {
      ...
      await cp(join(sourceRoot, path), destination, { recursive: true, errorOnExist: true });
      const legacyManifest = join(destination, "manifest.json");
