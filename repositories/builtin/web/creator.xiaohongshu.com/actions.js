...
window.ox.install(2, ({ action }) => {
    ...
    const captureData = async (pattern, label, replayLatest) => {
        const capture = waitForCapture;
        if (!capture)
            throw new Error("response capture is unavailable");
        console.log(`capture start endpoint=${label} replay=${replayLatest}`);
        ...
        console.log(`capture success endpoint=${label}`);
    ...
    const dialog = async (text) => waitFor(() => Array.from(document.querySelectorAll("[role='dialog'], .d-modal, .el-dialog"))
        .filter(visible)
        .find((element) => text.test(element.textContent ?? "")));
    const confirmDialog = (container) => {
        const button = Array.from(container.querySelectorAll("button"))
            .filter(visible)
            .find((candidate) => /^(确定|确认)$/.test(candidate.textContent?.trim() ?? ""));
    ...
    const captureMutation = async (pattern, label) => {
        ...
        if (!capture)
            throw new Error("response capture is unavailable");
        console.log(`mutation capture start endpoint=${label}`);
        ...
        console.log(`mutation success endpoint=${label}`);
        return response;
    };
    ...
    const currentUserId = () => String(document.querySelector("#app")?.__vue_app__?.config
        ?.globalProperties?.$store?.state?.Auth?.userInfo?.userId ??
        window.publish?.getUserState?.()?.userInfo?.value?.id ??
        "");
    const draftTitle = (content) => String(content?.articleTitle ??
        content?.draftStore?.title ??
        content?.title ??
        "");
    const draftDescription = (content) => String(content?.shortTextContent ??
        content?.draftStore?.desc ??
        content?.description ??
        content?.desc ??
        "");
    const draftCover = (content) => {
        const cover = content?.draftStore?.coverUrl ??
    ...
    action("getSignInState", {
        async invoke() {
            ...
            const signedIn = value?.success === true &&
                number(value?.code) === 0 &&
                value?.data !== null &&
                    ...
                    topics: (Array.isArray(group.selectTopics)
                        ...
                        exampleNotes: (Array.isArray(topic.notes) ? topic.notes : []).map((note) => ({
                            ...
                            images: (Array.isArray(note.images_list)
                                ? note.images_list
                                : [])
                                .map((image) => String(image?.url ?? image ?? ""))
                                .filter(Boolean),
                        })),
                    })),
                })),
    ...
    action("listCreationGuidance", {
        async invoke({ category = "official", cursor, } = {}) {
            const categories = {
    ...
    action("listCreatorDrafts", {
        async invoke() {
            ...
            database.close();
            const items = records
                .flatMap(({ store, items: values }) => values
    ...
    action("updateNoteVisibility", {
        async invoke({ noteId, visibility, }) {
            const labels = {
                public: "公开可见",
                private: "仅自己可见",
                friends: "仅互关好友可见",
            ...
            select.click();
            const option = await waitFor(() => Array.from(document.querySelectorAll(".d-popover .custom-option"))
                .filter(visible)
