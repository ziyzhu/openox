...
Use the `sim-cli` skill for visible iOS interaction and the `ox-cli` skill to verify the selected chat, model, and completed replies. Follow the repository's simulator rules. Keep recordings and response drafts outside the repository.

## Prepare

1. Use the requested simulator, normally `ox-qa-1`, and confirm the existing Ox app is ready with bundled services.
