...
4. Claim only fixed `ox-qa-1` through `ox-qa-5` simulators. Give every concurrent process its own numbered simulator and matching service and debug ports from `tooling/qa-config.ts`; use its repository port only for an explicit repository test.
