...
manifests retain verified public favicon URLs when present; otherwise they use
`openox.ai` CloudFront URLs backed by a private S3 bucket. Bundled PNGs provide
an on-device fallback when a built-in icon request fails. Icon requests use an
