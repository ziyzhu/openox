...
function source(faviconUrl?: string, withAsset = false): string {
  ...
  if (withAsset) writeFileSync(join(directory, "favicon.png"), "legacy asset");
  return root;
}

test("built-in service preserves its verified favicon URL", async () => {
...
test("built-in service retains the hosted URL for an existing icon asset", async () => {
...
test("built-in service rejects an invalid favicon URL", async () => {
