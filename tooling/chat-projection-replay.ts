...
async function replay(args: string[]): Promise<void> {
  let fixtureDirectory = join(ROOT, "apps/ios/fixtures/chatlogs");
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (argument === "--fixtures") fixtureDirectory = resolve(ROOT, takeValue(args, index++, argument));
      ...
      console.log("Usage: bun run test:chat-projection [--fixtures apps/ios/fixtures/chatlogs] [--update] [--json] [--timeout 30000]");
      return;
